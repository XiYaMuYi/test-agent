...
import {
  ...
  type RawMessageRow,
} from '../src/jobs/transcript-loader.js';
...
function responseHash(payload: Record<string, unknown>): string {
...
test('buildTranscriptFromMessages picks the last customerMood seen across turns', () => {
  const result = buildTranscriptFromMessages([
    row(
      1,
      '你好',
      responseHash({ suggestion: { replyText: '你好' }, customerMood: 'neutral' }),
    ),
    row(
      2,
      '你们的东西太贵了',
      responseHash({ suggestion: { replyText: '抱歉' }, customerMood: 'negative' }),
    ),
    row(
      3,
      '好吧，我再看看',
      responseHash({ suggestion: { replyText: '好的' }, customerMood: 'positive' }),
    ),
  ]);
