...
test('readModelConfig reads MODEL_BASE_URL from the environment', async () => {
  ...
  try {
    ...
    if (saved === undefined) delete process.env.MODEL_BASE_URL;
    else process.env.MODEL_BASE_URL = saved;
  }
});
...
test('readModelConfig reads MODEL_TIMEOUT_MS from the environment', async () => {
  ...
  try {
    ...
    if (saved === undefined) delete process.env.MODEL_TIMEOUT_MS;
    else process.env.MODEL_TIMEOUT_MS = saved;
  }
});
