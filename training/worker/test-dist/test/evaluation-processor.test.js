...
import { TransactionalEvaluationProcessor, } from '../src/jobs/evaluation.processor.js';
/**
...
class FakeDatabase {
    ...
    async query(statement, params) {
        if (statement.includes('INSERT INTO evaluation_report') && Array.isArray(params)) {
            try {
                this.persistedReport = JSON.parse(String(params[4]));
            }
            catch {
...
function buildFakeDatabase() {
    const responseHash = (payload) => JSON.stringify(payload);
...
test('loadEvaluationInput populates transcript, personaConfig and customerMood on EvaluationInput', async () => {
    ...
    const processor = new TransactionalEvaluationProcessor(buildFakeDatabase(), capturingGenerator, { leaseDurationMs: 60_000 });
    ...
    assert.equal(input.personaConfig?.conversation?.productScenario, '护肤品推荐');
