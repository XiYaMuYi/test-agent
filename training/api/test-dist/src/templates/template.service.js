...
import { DEFAULT_LEARNER_OVERRIDE_POLICY, diffTemplateRevisions as diffRevisions, normalizeAgentConfig, normalizeLearnerOverridePolicy, validateAgentConfigV1, validateLearnerOverridePolicy, } from '@training/contracts';
...
const SELECT_COLUMNS = `
  ...
  t.learner_override_policy, t.status, t.created_at,
...
export class TemplateService {
    ...
    async listVisibleTemplates(principal, filter = {}) {
        ...
        const { rows } = await this.database.query(`SELECT ${SELECT_COLUMNS}
       FROM training_template t
       WHERE ${conditions.join(' AND ')}
       ORDER BY t.recommended DESC, t.scope ASC, t.created_at DESC`, values);
        return rows.map(toView);
    }
    /** Resolve a template a learner may start a free session against; hidden/archived/foreign look identical. */
    async resolveVisibleTemplate(principal, templateId) {
        const learnerId = deriveLearnerId(principal.principalId);
        const { rows } = await this.database.query(`SELECT ${SELECT_COLUMNS}
       FROM training_template t
       WHERE t.id = $3 AND t.status = 'active'
         AND (t.scope = 'platform'
              OR (t.scope = 'organization' AND t.organization_id = $1 AND t.visible = TRUE)
              OR (t.scope = 'personal' AND t.owner_learner_id = $2))`, [principal.organizationId, learnerId, templateId]);
        const row = rows[0];
        if (row === undefined) {
    ...
    async createPersonalTemplate(principal, input) {
        const learnerId = deriveLearnerId(principal.principalId);
     ...
     */
    async createOrganizationTemplate(principal, input) {
        const client = await this.database.connect();
        try {
            ...
            await client.query(`INSERT INTO training_template_revision (
    ...
    async listOrganizationTemplates(principal, statusFilter = 'active') {
       ...
       FROM training_template t
       WHERE ${conditions.join(' AND ')}
       ORDER BY t.created_at DESC, t.id ASC`, values);
        return rows.map(toView);
    }
    /**
     ...
     */
    async updateOrganizationTemplate(principal, templateId, patch) {
        const client = await this.database.connect();
        try {
            await client.query('BEGIN');
            const current = await client.query(`SELECT ${SELECT_COLUMNS}
         FROM training_template t
         WHERE organization_id = $1 AND id = $2 AND scope = 'organization'
         FOR UPDATE`, [principal.organizationId, templateId]);
            ...
            await client.query(`UPDATE training_template SET
           title = $1, persona_config = $2::jsonb, knowledge_versions = $3::jsonb,
           scoring_rules = $4::jsonb, agent_config = $5::jsonb,
           learner_override_policy = $6::jsonb, updated_at = CURRENT_TIMESTAMP
         WHERE organization_id = $7 AND id = $8 AND scope = 'organization'`, [
                ...
                JSON.stringify(next.learnerOverridePolicy), principal.organizationId, templateId,
            ]);
    ...
    async duplicateOrganizationTemplate(principal, templateId) {
        const client = await this.database.connect();
        try {
            await client.query('BEGIN');
            const current = await client.query(`SELECT ${SELECT_COLUMNS}
         ...
         FOR UPDATE`, [principal.organizationId, templateId]);
            ...
            await client.query(`INSERT INTO training_template (
    ...
    async setOrganizationTemplateStatus(principal, templateId, status) {
        const { rows } = await this.database.query(`UPDATE training_template SET status = $1, updated_at = CURRENT_TIMESTAMP
       WHERE organization_id = $2 AND id = $3 AND scope = 'organization'
       RETURNING ${SELECT_COLUMNS.replace(/\bt\./g, '')}`, [status, principal.organizationId, templateId]);
        const row = rows[0];
        if (row === undefined)
            throw new Error('TEMPLATE_NOT_FOUND');
        return toView(row);
    ...
    async listTemplateRevisions(principal, templateId) {
        const { rows } = await this.database.query(`SELECT id AS "revisionId", template_id AS "templateId", revision, title,
       ...
       ORDER BY revision DESC`, [principal.organizationId, templateId]);
        return rows.map((row) => ({
            ...row,
    ...
    async getTemplateRevision(principal, templateId, revisionId) {
        const { rows } = await this.database.query(`SELECT r.id AS "revisionId", r.template_id AS "templateId", r.revision, r.title,
       ...
       WHERE r.organization_id = $1 AND r.template_id = $2 AND r.id = $3`, [principal.organizationId, templateId, revisionId]);
        const row = rows[0];
        if (row === undefined)
    ...
    async listTemplateScenarioBindings(principal, templateId) {
        const { rows } = await this.database.query(`SELECT d.id AS draft_id, d.title,
       ...
       WHERE d.organization_id = $1
         ...
         AND d.payload -> 'personaSource' ->> 'templateId' = $2`, [principal.organizationId, templateId]);
        return rows.map((row) => ({
    ...
    async diffTemplateRevisions(principal, templateId, revisionA, revisionB) {
        const { rows } = await this.database.query(`SELECT persona_config, knowledge_versions, scoring_rules, agent_config, learner_override_policy
       FROM training_template_revision
       WHERE organization_id = $1 AND template_id = $2 AND revision IN ($3, $4)
       ORDER BY revision ASC`, [principal.organizationId, templateId, revisionA, revisionB]);
        if (rows.length !== 2)
            throw new Error('TEMPLATE_REVISION_NOT_FOUND');
        ...
        const before = {
            ...
            revision: revisionA,
            title: '',
            personaConfig: older.persona_config,
            knowledgeVersions: older.knowledge_versions ?? [],
            scoringRules: older.scoring_rules ?? [],
        ...
        const after = {
            ...before,
            revision: revisionB,
            personaConfig: newer.persona_config,
            knowledgeVersions: newer.knowledge_versions ?? [],
            scoringRules: newer.scoring_rules ?? [],
    ...
    async requireOrganizationTemplate(principal, templateId) {
        const { rows } = await this.database.query(`SELECT ${SELECT_COLUMNS}
       FROM training_template t
       WHERE organization_id = $1 AND id = $2 AND scope = 'organization'`, [principal.organizationId, templateId]);
        const row = rows[0];
        if (row === undefined)
            throw new Error('TEMPLATE_NOT_FOUND');
        return toView(row);
    }
    validatePersonaOrThrow(raw) {
        const validation = this.personas.validatePersonaConfig(raw);
        if (!validation.valid)
            throw new Error('PERSONA_CONFIG_INVALID');
        return raw;
    }
    validateAgentConfigOrThrow(raw) {
        const validation = validateAgentConfigV1(raw);
        if (!validation.valid)
            throw new Error('AGENT_CONFIG_INVALID');
        return raw;
    }
    validatePolicyOrThrow(raw) {
        const validation = validateLearnerOverridePolicy(raw);
        if (!validation.valid)
            throw new Error('LEARNER_OVERRIDE_POLICY_INVALID');
        return raw;
    }
    async ensureLearnerProfile(client, principal, learnerId) {
        await client.query(`INSERT INTO learner_profile (internal_learner_id, organization_id, external_principal_id, identity_provider)
       VALUES ($1, $2, $3, 'gongzhugou')
       ON CONFLICT (organization_id, internal_learner_id) DO NOTHING`, [learnerId, principal.organizationId, principal.principalId]);
    }
    ...
    async insertTemplate(client, organizationId, scope, ownerLearnerId, input) {
        ...
        try {
            const { rows } = await client.query(`INSERT INTO training_template (
           id, organization_id, scope, owner_learner_id, title, persona_config,
...
function toRevisionRow(view, principal, revision) {
    return {
        ...
        knowledgeVersions: view.knowledgeVersions,
        scoringRules: view.scoringRules,
