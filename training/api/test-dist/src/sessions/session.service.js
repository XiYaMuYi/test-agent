...
import { buildDefaultConfig, normalizeAgentConfig } from '@training/contracts';
import { AssignmentProblem, deriveLearnerId, EligibilityService } from '../assignments/eligibility.service.js';
import { parseOverridePatch } from '../assignments/assignment.service.js';
import { assertOrganizationScope } from '../identity/organization-context.js';
const IN_FLIGHT_STATUSES = ['created', 'active'];
...
export class SessionService {
    ...
    eligibility;
    templates;
    constructor(database, personas, eligibility, templates) {
    ...
    async startFreeSession(principal, input, idempotencyKey) {
        ...
        try {
            ...
            await this.upsertLearnerProfile(client, principal, learnerId, personaSnapshot.name);
            // Idempotent replay takes precedence over the single-active guard.
            const existing = await client.query(`SELECT ts.id, c.id AS conversation_id
         FROM training_session ts
         LEFT JOIN conversation c
           ON c.organization_id = ts.organization_id
          AND c.training_session_id = ts.id
         WHERE ts.organization_id = $1 AND ts.learner_id = $2 AND ts.idempotency_key = $3`, [organizationId, learnerId, idempotencyKey]);
            if (existing.rows[0] !== undefined) {
                const row = existing.rows[0];
                let conversationId = row.conversation_id;
                if (conversationId === null) {
                    conversationId = crypto.randomUUID();
                    await client.query(`INSERT INTO conversation (id, organization_id, training_session_id, status)
             VALUES ($1, $2, $3, 'created')`, [conversationId, organizationId, row.id]);
                }
            ...
            await client.query(`INSERT INTO training_session (
         ...
         VALUES ($1, $2, $3, 'free', $4, $7, NULL, NULL, NULL, NULL, $5::jsonb, 'created', $6)`, [sessionId, organizationId, learnerId, mode, JSON.stringify(personaSnapshot), idempotencyKey, resolvedTemplate?.id ?? null]);
         ...
         VALUES ($1, $2, NULL, NULL, $3, 'created')`, [conversationId, organizationId, sessionId]);
            await client.query('COMMIT');
    ...
    async startAssignedSession(principal, assignmentId, idempotencyKey) {
        const learnerId = deriveLearnerId(principal.principalId);
        const organizationId = principal.organizationId;
        const client = await this.database.connect();
        try {
            await client.query('BEGIN');
            const assignmentResult = await client.query(`SELECT organization_id, release_snapshot_id, status, starts_at, ends_at, max_attempts, override_patch
         FROM assignment WHERE id = $1`, [assignmentId]);
            ...
            const learnerResult = await client.query(`SELECT id, state, total_attempts, active_attempt_id
         FROM learner_assignment
         WHERE organization_id = $1 AND assignment_id = $2 AND learner_id = $3
         FOR UPDATE`, [organizationId, assignmentId, learnerId]);
            ...
            const existing = await client.query(`SELECT attempt.id, attempt.learner_assignment_id,
         ...
         WHERE attempt.organization_id = $1 AND attempt.idempotency_key = $2`, [organizationId, idempotencyKey]);
            if (existing.rows[0] !== undefined) {
                const row = existing.rows[0];
                ...
                if (conversationId === null) {
                    conversationId = crypto.randomUUID();
                    await client.query(`INSERT INTO conversation (id, organization_id, training_attempt_id, release_snapshot_id, training_session_id, status)
             VALUES ($1, $2, $3, $4, $5, 'created')`, [conversationId, organizationId, row.id, assignment.release_snapshot_id, row.session_id]);
                }
                await client.query('COMMIT');
            ...
            const snapshotResult = await client.query(`SELECT snapshot FROM release_snapshot
         WHERE id = $1 AND organization_id = $2`, [assignment.release_snapshot_id, organizationId]);
         ...
         VALUES ($1, $2, $3, $4, 'created')`, [attemptId, organizationId, learnerAssignment.id, idempotencyKey]);
         ...
         VALUES ($1, $2, $3, $4, $5, 'created')`, [conversationId, organizationId, attemptId, assignment.release_snapshot_id, sessionId]);
            await client.query(`UPDATE learner_assignment
         SET active_attempt_id = $1, state = 'active', updated_at = CURRENT_TIMESTAMP
         WHERE id = $2 AND organization_id = $3`, [attemptId, learnerAssignment.id, organizationId]);
            await client.query('COMMIT');
    ...
    async assertNoInFlightSession(client, organizationId, learnerId, code) {
        const active = await client.query(`SELECT 1 FROM training_session
       WHERE organization_id = $1 AND learner_id = $2 AND status = ANY($3)
       LIMIT 1`, [organizationId, learnerId, IN_FLIGHT_STATUSES]);
    ...
    async upsertLearnerProfile(client, principal, learnerId, displayName) {
        await client.query(`INSERT INTO learner_profile (
       ...
       SET external_principal_id = EXCLUDED.external_principal_id,
           ...
           updated_at = CURRENT_TIMESTAMP`, [learnerId, principal.organizationId, principal.principalId, displayName]);
    }
    /** Freeze a stored/template persona after validating it; reject malformed configs. */
    freezeValidPersona(raw) {
    ...
    resolveTaskPersona(snapshot) {
        ...
        if (schemaVersion === 'release-snapshot/v2') {
            if (raw === undefined || raw === null || !this.personas.validatePersonaConfig(raw).valid) {
                throw new AssignmentProblem('RELEASE_SNAPSHOT_INVALID', HttpStatus.UNPROCESSABLE_ENTITY, 'The v2 release snapshot does not carry a valid personaConfig; refusing to start the assigned session.');
            }
 ...
 */
function applyPersonaOverride(persona, override) {
    if (override?.conversation === undefined)
        return persona;
    const { maxTurns, openingMode, background } = override.conversation;
    if (maxTurns === undefined && openingMode === undefined && background === undefined)
        return persona;
    return {
        ...persona,
        conversation: {
 ...
 */
function applyAgentConfigOverride(snapshotAgentConfig, override) {
