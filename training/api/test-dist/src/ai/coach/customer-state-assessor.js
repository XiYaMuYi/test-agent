 ...
 */
import { CUSTOMER_STATE_KEYS, customerMoodFromState, } from '@training/contracts';
/**
...
const clamp0100 = (value) => Math.max(0, Math.min(100, value));
function personaBrief(persona) {
    ...
    if (typeof c.skinType === 'string')
        parts.push(`肤质 skinType=${c.skinType}`);
    return parts.length > 0 ? parts.join('，') : '（人设未提供量化基线）';
}
/** 构造让大模型研判“本轮结束后客户 12 维状态绝对值”的 prompt。 */
export function buildCustomerStatePrompt(params) {
 ...
 */
export function parseCustomerStateAssessment(raw, previous) {
...
export async function assessCustomerState(generate, params) {
    try {
...
export function moodLabelOf(state) {
