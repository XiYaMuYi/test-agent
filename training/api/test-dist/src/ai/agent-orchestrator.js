...
export class AgentOrchestrator {
    knowledge;
    model;
    constructor(knowledge, model) {
        this.knowledge = knowledge;
        this.model = model;
    }
    async buildAndValidateContext(conversation, snapshot, recentMessages) {
    ...
    async generateDecision(context, mode = 'reply') {
        ...
        if (mode === 'reply' && !decision.stateTransition)
            decision = { ...decision, stateTransition: fallbackCustomerTransition(decision, context) };
    ...
    buildPrompt(context, mode = 'reply') {
    ...
    validateAgentDecision(value, context) {
        ...
        for (const ref of obj.knowledgeReferences) {
        ...
        if (obj.stateTransition != null) {
            try {
                stateTransition = validateCustomerTransition(obj.stateTransition, context);
            }
            catch {
                throw new OrchestratorError('MODEL_SCHEMA_INVALID', 'Invalid customer state transition');
            }
        }
        return {
            ...
            knowledgeReferences: obj.knowledgeReferences,
            confidence: obj.confidence,
            ...(stateTransition ? { stateTransition } : {}),
        };
    }
    async loadApprovedKnowledge(knowledgeVersions, organizationId) {
        const entries = [];
 ...
 */
function buildAgentBehaviorBlock(agentConfig) {
    if (agentConfig === undefined || agentConfig === null || typeof agentConfig !== 'object')
        return '';
    ...
    switch (responseLength) {
        case 'short':
            lines.push('- 回复尽量简短，通常一句话以内。');
            break;
        case 'detailed':
            lines.push('- 可以回复 2-3 句话，必要时展开说明。');
            break;
        default:
            lines.push('- 每次回复 1-2 句话，保持自然。');
            break;
    }
    switch (knowledgeStrictness) {
        case 'strict':
            lines.push('- 只有产品知识条目明确包含时才提及成分、功效等细节，绝不编造知识。');
            break;
        default:
            lines.push('- 可结合常识自然回应，但不得编造产品知识条目之外的新信息。');
            break;
    }
    switch (conversationPace) {
        case 'slow':
            lines.push('- 推进缓慢：需要销售员多次引导才深入话题，不急于进入实质讨论。');
            break;
        case 'fast':
            lines.push('- 推进较快：销售员引导几次后即可进入实质讨论。');
            break;
        default:
            lines.push('- 按正常节奏对话，跟随销售员的引导自然推进。');
            break;
    }
    switch (closingTendency) {
        case 'resistant':
            lines.push('- 成交倾向低：需要销售员强力说服才考虑购买，常见疑虑较多。');
            break;
        case 'receptive':
            lines.push('- 成交倾向高：较容易被说服，条件合适时倾向成交。');
            break;
        default:
            lines.push('- 成交倾向中性：是否购买视对话情况自然决定。');
            break;
    }
    if (lines.length === 0 && (additionalInstructions === undefined || additionalInstructions.length === 0))
        return '';
