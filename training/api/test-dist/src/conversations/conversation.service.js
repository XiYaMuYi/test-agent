...
import { canEndConversation, decideMessage } from './conversation-state.js';
...
export class ConversationService {
    ...
    async getConversation(principal, conversationId) {
        const { rows } = await this.database.query(`SELECT c.id, c.organization_id AS "organizationId", c.status, c.version, c.last_sequence AS "lastSequence",
       ...
       WHERE c.id = $1 AND c.organization_id = $2 AND ts.learner_id = $3`, [conversationId, principal.organizationId, deriveLearnerId(principal.principalId)]);
        return rows[0];
    }
    /**
     ...
     */
    async listForLearner(principal, pagination) {
       ...
       LEFT JOIN evaluation_report er ON er.conversation_id = c.id AND er.organization_id = c.organization_id`;
        ...
        const { rows } = await this.database.query(`SELECT c.id AS "conversationId", c.status, ts.source_type AS "sourceType", ts.mode,
       ...
       LIMIT $3 OFFSET $4`, [principal.organizationId, learnerId, pagination.limit, pagination.offset]);
        const count = await this.database.query(`SELECT COUNT(*)::int AS n ${fromSql} ${whereSql}`, [principal.organizationId, learnerId]);
     ...
     */
    async getConversationDetail(principal, conversationId) {
        const meta = await this.getConversation(principal, conversationId);
        if (meta === undefined)
            return undefined;
        const { rows } = await this.database.query(`SELECT sequence, role, content, client_message_id AS "clientMessageId", response_hash AS "responseHash"
       FROM conversation_message
       WHERE conversation_id = $1 AND organization_id = $2
       ORDER BY sequence ASC`, [conversationId, principal.organizationId]);
        const transcript = [];
        ...
        const reportRow = await this.database.query(`SELECT status, report FROM evaluation_report
       WHERE conversation_id = $1 AND organization_id = $2
       ORDER BY created_at DESC LIMIT 1`, [conversationId, principal.organizationId]);
    ...
    async startConversation(principal, conversationId) {
        const conversation = await this.getConversation(principal, conversationId);
        if (conversation === undefined)
            return undefined;
        const saved = await this.database.query(`SELECT opening_response AS "openingResponse"
       FROM conversation
       WHERE id = $1 AND organization_id = $2`, [conversationId, principal.organizationId]);
        if (saved.rows[0]?.openingResponse)
            return saved.rows[0]?.openingResponse;
        ...
        if (conversation.personaSnapshot?.conversation?.openingMode === 'wait_learner') {
            const learnerFirstOpening = { opening: '', customerMood: 'neutral', learnerFirst: true };
            return this.persistOpening(principal, conversationId, learnerFirstOpening, initialState);
        }
        if (this.orchestrator === undefined)
            throw new Error('MODEL_UPSTREAM_UNAVAILABLE');
     ...
     */
    async persistOpening(principal, conversationId, opening, initialState) {
        const persisted = await this.database.query(`UPDATE conversation
       ...
       RETURNING opening_response AS "openingResponse"`, [JSON.stringify(opening), conversationId, principal.organizationId, JSON.stringify(initialState)]);
        return persisted.rows[0]?.openingResponse;
    }
    async sendMessage(principal, conversationId, input) {
        ...
        try {
            ...
            if (decision.kind === 'replay' && existing !== undefined) {
                ...
                await client.query(`INSERT INTO conversation_message (id, organization_id, conversation_id, sequence, client_message_id, role, content, request_hash)
           VALUES ($1, $2, $3, $4, $5, 'learner', $6, $7)`, [messageId, principal.organizationId, conversation.id, input.sequence, input.clientMessageId, input.content, hashMessage(input)]);
                await client.query(`UPDATE conversation
           SET status = 'awaiting_model',
               version = version + 1,
               last_sequence = $1,
               updated_at = CURRENT_TIMESTAMP
           WHERE id = $2 AND organization_id = $3`, [input.sequence, conversation.id, principal.organizationId]);
        ...
        const context = await this.orchestrator.buildAndValidateContext({ ...conversation, status: 'awaiting_model', version: processingVersion, lastSequence: input.sequence }, knowledgeContext, mappedMessages);
        ...
        const previousState = context.currentCustomerState ?? createInitialCustomerState(conversation.personaSnapshot);
        const customerMood = customerMoodFromState(previousState);
        ...
        const enrichedSuggestion = {
            ...suggestion,
            ...(shouldAutoEnd ? { suggestedAction: 'end' } : {}),
            customerMood,
        };
        const response = {
            ...baseResponse,
    ...
    async getCoachFeedback(principal, conversationId, messageId) {
        const { rows } = await this.database.query(`SELECT message.response_hash::text AS "responseHash"
       ...
       WHERE message.id = $1 AND message.conversation_id = $2
         AND message.organization_id = $3 AND ts.learner_id = $4`, [messageId, conversationId, principal.organizationId, deriveLearnerId(principal.principalId)]);
    ...
    async generateAndPersistCoachFeedback(input) {
        let feedback = input.fallback;
        // 优先用 LLM 生成有上下文理解能力的教练点评；模型不可用或输出非法时
        // llmCoachFeedback 内部会回落到规则引擎，因此这里永远能得到可用结果。
        if (this.orchestrator !== undefined) {
            const llmResult = await llmCoachFeedback((prompt) => this.orchestrator.generateRawText(prompt), {
        ...
        const finalFeedback = { ...feedback, provisional: false };
        await this.database.query(`UPDATE conversation_message
       SET response_hash = jsonb_set(response_hash::jsonb, '{coachFeedback}', $1::jsonb, true)
       WHERE id = $2 AND organization_id = $3 AND response_hash IS NOT NULL`, [JSON.stringify(finalFeedback), input.messageId, input.organizationId]);
    }
    /**
     * 客户状态轮询接口：异步大模型研判写回（customerStatePending=false）前一直返回 pending，
     * 写回后返回 ready 与本轮研判出的 12 维状态/情绪，前端据此停止轮询并延迟上屏。
     */
    async getCustomerState(principal, conversationId, messageId) {
        const { rows } = await this.database.query(`SELECT message.response_hash::text AS "responseHash"
       FROM conversation_message message
       JOIN conversation c ON c.id = message.conversation_id AND c.organization_id = message.organization_id
       JOIN training_session ts ON ts.id = c.training_session_id AND ts.organization_id = c.organization_id
     ...
     */
    async generateAndPersistCustomerState(input) {
        if (this.orchestrator === undefined)
            return;
        const assessed = await assessCustomerState((prompt) => this.orchestrator.generateRawText(prompt), { persona: input.persona, previous: input.previous, recentMessages: input.recentMessages, turnCount: input.turnCount });
        if (assessed === null)
            return;
       ...
       WHERE id = $3 AND organization_id = $4 AND response_hash IS NOT NULL`, [JSON.stringify(assessed), JSON.stringify(mood), input.messageId, input.organizationId]);
        await this.database.query(`UPDATE conversation c
       ...
       WHERE m.id = $2 AND m.conversation_id = c.id AND c.organization_id = $3
         AND m.sequence = c.last_sequence`, [JSON.stringify(assessed), input.messageId, input.organizationId]);
    }
    async finalizeMessage(principal, expectedVersion, response) {
        ...
        try {
            ...
            if (reason)
                response = { ...response, status: 'ended', version: expectedVersion + 1, endReason: reason };
            const persisted = await client.query(`UPDATE conversation_message
         SET response_hash = $1
         WHERE id = $2 AND organization_id = $3 AND response_hash IS NULL`, [JSON.stringify(response), response.messageId, organizationId]);
            ...
            const advanced = await client.query(`UPDATE conversation
         SET status = $5,
             version = $6,
             end_reason = $7,
             ...
             updated_at = CURRENT_TIMESTAMP
         WHERE id = $1 AND organization_id = $2 AND version = $3 AND status = 'awaiting_model'`, [response.conversationId, organizationId, expectedVersion, response.customerState ? JSON.stringify(response.customerState) : null, response.status, response.version, reason, JSON.stringify(createInitialCustomerState(conversation.personaSnapshot))]);
            if (advanced.rowCount !== 1) {
                throw new Error('MESSAGE_SEQUENCE_CONFLICT');
            }
            if (reason)
                await this.settleConversation(client, conversation, organizationId);
            await client.query('COMMIT');
            return toClientMessageResponse(response);
        }
        catch (error) {
    ...
    async endConversation(principal, conversationId, reason = 'manual_end') {
        const client = await this.database.connect();
        try {
            await client.query('BEGIN');
            ...
            await client.query(`UPDATE conversation SET status = 'ended', end_reason = $3, version = version + 1, updated_at = CURRENT_TIMESTAMP WHERE id = $1 AND organization_id = $2`, [conversation.id, principal.organizationId, reason]);
    ...
    async settleConversation(client, conversation, organizationId) {
        ...
        await client.query(`UPDATE training_session
         SET status = 'ended', finished_at = COALESCE(finished_at, CURRENT_TIMESTAMP)
         WHERE id = $1 AND organization_id = $2 AND status IN ('created', 'active')`, [conversation.trainingSessionId, organizationId]);
        ...
        if (conversation.sourceType === 'assigned') {
            ...
            const settlement = await client.query(`WITH settled_attempt AS (
           ...
           WHERE learner.id = attempt.learner_assignment_id
             ...
             AND learner.active_attempt_id = $1`, [conversation.trainingAttemptId, organizationId]);
            if (settlement.rowCount !== 1) {
                throw new Error('ATTEMPT_SETTLEMENT_CONFLICT');
            }
        }
        const evaluationJobId = crypto.randomUUID();
        await client.query(`INSERT INTO evaluation_job (id, organization_id, conversation_id, status, attempt_count)
         VALUES ($1, $2, $3, 'queued', 0)
         ON CONFLICT (organization_id, conversation_id) DO NOTHING`, [evaluationJobId, organizationId, conversation.id]);
        await client.query(`INSERT INTO outbox_event (id, organization_id, aggregate_type, aggregate_id, event_type, payload, deduplication_key, status)
    ...
    async lockConversation(client, principal, conversationId) {
        const { rows } = await client.query(`SELECT c.id, c.organization_id AS "organizationId", c.status, c.version, c.last_sequence AS "lastSequence",
              c.training_attempt_id AS "trainingAttemptId", c.release_snapshot_id AS "releaseSnapshotId",
               c.training_session_id AS "trainingSessionId", ts.source_type AS "sourceType",
                ts.persona_snapshot AS "personaSnapshot",
                c.opening_response AS "openingResponse",
       ...
       FOR UPDATE`, [conversationId, principal.organizationId, deriveLearnerId(principal.principalId)]);
    ...
    async findExistingMessage(client, conversationId, clientMessageId) {
        const { rows } = await client.query(`SELECT id, conversation_id AS "conversationId", client_message_id AS "clientMessageId", sequence, content, request_hash AS "requestHash", response_hash AS "responseHash"
       FROM conversation_message
       WHERE conversation_id = $1 AND client_message_id = $2`, [conversationId, clientMessageId]);
        return rows[0];
    }
    async findMessageById(client, messageId, organizationId) {
        const { rows } = await client.query(`SELECT id, conversation_id AS "conversationId", client_message_id AS "clientMessageId", sequence, content, request_hash AS "requestHash", response_hash AS "responseHash"
       FROM conversation_message
       WHERE id = $1 AND organization_id = $2`, [messageId, organizationId]);
        return rows[0];
    }
    async loadKnowledgeContext(conversation) {
        ...
        if (conversation.trainingSessionId) {
            const { rows } = await this.database.query('SELECT agent_config FROM training_session WHERE id = $1', [conversation.trainingSessionId]);
            ...
            if (sessionAgent !== null && sessionAgent !== undefined && typeof sessionAgent === 'object' && !Array.isArray(sessionAgent)) {
                ...
                return {
                    ...
                    agentConfig: normalizeAgentConfig(sessionAgent),
                };
            }
        }
    ...
    async loadReleaseSnapshot(releaseSnapshotId) {
    ...
    async loadRecentMessages(conversationId, limit = 20) {
        const { rows } = await this.database.query(`SELECT id, conversation_id AS "conversationId", client_message_id AS "clientMessageId", sequence, content, request_hash AS "requestHash", response_hash AS "responseHash"
       FROM conversation_message
...
function extractAssistantReply(serializedResponse) {
    try {
...
function toCoachConversationContext(messages, openingResponse) {
    const opening = typeof openingResponse?.opening === 'string' ? openingResponse.opening.trim() : '';
    if (opening === '' || openingResponse?.learnerFirst === true)
        return messages;
    return [{ role: 'assistant', content: opening }, ...messages];
}
function findPreviousCustomerMessage(messages) {
    let learnerIndex = -1;
...
function parsePersistedMessageResponse(serialized, message, conversationId) {
    let value;
    try {
...
function isAgentOutputV1(value) {
    if (typeof value !== 'object' || value === null)
        return false;
    const output = value;
 ...
 */
export function toClientMessageResponse(response) {
