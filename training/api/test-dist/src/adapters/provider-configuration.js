...
function readPositiveTimeout(value, name, fallback) {
...
function readModelProvider(env) {
...
function readKnowledgeProvider(env) {
    const kind = (env.KNOWLEDGE_PROVIDER?.trim() || 'fake');
    if (kind !== 'fake' && kind !== 'external' && kind !== 'weknora') {
        throw new Error(`KNOWLEDGE_PROVIDER must be "fake", "external" or "weknora" (got "${env.KNOWLEDGE_PROVIDER}").`);
    }
    if (kind === 'fake')
    ...
    if (kind === 'external') {
        ...
        return {
            ...
            externalKnowledge: {
                ...
                timeoutMs: readPositiveTimeout(env.EXTERNAL_KNOWLEDGE_TIMEOUT_MS, 'EXTERNAL_KNOWLEDGE_TIMEOUT_MS', DEFAULT_KNOWLEDGE_TIMEOUT_MS),
            },
 ...
 */
export function readAiProviderConfiguration(environment = process.env) {
