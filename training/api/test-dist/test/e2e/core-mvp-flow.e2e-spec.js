...
async function loadWorkerModule(connectionString) {
...
// ---------------------------------------------------------------------------
test('P1-10: core MVP end-to-end flow from scenario draft to evaluation report', { timeout: 180_000 }, async (context) => {
    // -----------------------------------------------------------------------
    // 1. Infrastructure
    // -----------------------------------------------------------------------
    ...
    const snapshots = await db.query('SELECT id, snapshot FROM release_snapshot WHERE scenario_draft_id = $1', [draftId]);
    ...
    const outboxEvents = await db.query("SELECT event_type, status FROM outbox_event WHERE event_type = 'release.published' ORDER BY created_at");
    ...
    const assignmentRow = await db.query('SELECT organization_id FROM assignment WHERE id = $1', [assignmentId]);
    ...
    const myTask = myTasks.body.items.find((t) => t.assignmentId === assignmentId);
    ...
    const sameOrgOtherTasks = await request(http)
        ...
        .expect(200);
    const sameOrgOtherSeesTarget = sameOrgOtherTasks.body.items.find((t) => t.assignmentId === assignmentId);
    ...
    const crossOrgTasks = await request(http)
        ...
        .expect(200);
    const crossOrgTask = crossOrgTasks.body.items.find((t) => t.assignmentId === assignmentId);
    ...
    const conversationId = startAttempt.body.conversationId;
    ...
    const convRow = await db.query('SELECT training_attempt_id, release_snapshot_id, organization_id, status FROM conversation WHERE id = $1', [conversationId]);
    ...
    for (const msg of messages) {
        ...
        firstMessageResponses.push(res.body);
        // I-02: Capture each round's Agent suggestion for replay verification
        firstMessageSuggestions.push(res.body.suggestion);
    }
    ...
    for (let i = 0; i < firstMessageSuggestions.length; i++) {
        ...
        assert.ok(['ask_follow_up', 'advance', 'end'].includes(sug.suggestedAction), `Message ${i + 1} must have valid suggestedAction`);
    }
    ...
    const convAfterMessages = await db.query('SELECT status, last_sequence::text AS last_sequence FROM conversation WHERE id = $1', [conversationId]);
    ...
    const convEnded = await db.query('SELECT status FROM conversation WHERE id = $1', [conversationId]);
    ...
    const evalJobs = await db.query('SELECT id, status, conversation_id FROM evaluation_job WHERE conversation_id = $1', [conversationId]);
    ...
    const evalOutbox = await db.query("SELECT event_type, status, deduplication_key FROM outbox_event WHERE aggregate_id = $1 AND event_type = 'evaluation.requested'", [conversationId]);
    ...
    const reportsBeforeWorker = await db.query('SELECT COUNT(*)::text AS count FROM evaluation_report WHERE conversation_id = $1', [conversationId]);
    ...
    const outboxAfterWorker = await db.query("SELECT status FROM outbox_event WHERE aggregate_id = $1 AND event_type = 'evaluation.requested'", [conversationId]);
    ...
    const evalJobAfterWorker = await db.query('SELECT status FROM evaluation_job WHERE conversation_id = $1', [conversationId]);
    ...
    const reports = await db.query('SELECT id, organization_id, conversation_id, evaluation_job_id, report FROM evaluation_report WHERE conversation_id = $1', [conversationId]);
    ...
    const evalJobAfter = await db.query('SELECT status FROM evaluation_job WHERE id = $1', [reportRow.evaluation_job_id]);
    ...
    const reportPayload = reportRow.report;
    ...
    const reportsAfterSecondPoll = await db.query('SELECT COUNT(*)::text AS count FROM evaluation_report WHERE conversation_id = $1', [conversationId]);
    ...
    assert.equal(streamerReport.body.report.schemaVersion, 'evaluation-report/v1');
    // -----------------------------------------------------------------------
    ...
    const matchingReport = adminReports.body.items.find((r) => r.conversationId === conversationId);
    ...
    const adminBReports = await request(http)
        ...
        .set('x-access-token', ADMIN_B)
        .expect(200);
    const orgBSeesOrgA = adminBReports.body.items.some((r) => r.conversationId === conversationId);
    ...
    await request(http)
        ...
        .set('x-access-token', ADMIN_A)
        .expect(201);
    const snapshotsAfterRepublish = await db.query('SELECT id FROM release_snapshot WHERE scenario_draft_id = $1', [draftId]);
    ...
    assert.notEqual(snapshotAfterRepublishFirst.id, snapshotAfterRepublishSecond.id, 'Snapshots must have different IDs');
    ...
    const dbSnapshots = await db.query('SELECT snapshot FROM release_snapshot WHERE scenario_draft_id = $1 ORDER BY created_at ASC', [draftId]);
