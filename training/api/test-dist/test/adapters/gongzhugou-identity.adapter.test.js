...
test('verifies a first-party signed mini-program token through the marketplace current-user endpoint', async () => {
    let request;
    const adapter = new GongzhugouIdentityAdapter('11111111-1111-1111-1111-111111111111', 1_000, async (url, init) => {
    ...
    assert.equal((request?.init?.headers)['x-token'], 'marketplace-token');
    assert.equal((request?.init?.headers).device_id, 'device-id');
});
test('rejects a marketplace token that the current-user endpoint marks invalid', async () => {
    const adapter = new GongzhugouIdentityAdapter('11111111-1111-1111-1111-111111111111', 1_000, async () => (new Response(JSON.stringify({ code: 1101 }), { status: 200 })));
    await assert.rejects(() => adapter.verifyAccessToken('invalid-token', { openId: 'wx-open-id', deviceId: 'device-id' }), /invalid/i);
});
