...
import { createPostgresExecutor, MigrationRunner, schemaMigrationsBaseline, coreMvpSchemaMigration, workerEvaluationLeaseMigration, personaTemplatesMigration, cFirstDualTrackMigration, } from '../../src/database/migrations/index.js';
...
async function applyAll(connectionUri) {
...
test('0015 backfills legacy assigned conversations into assigned sessions', { timeout: 120_000 }, async () => {
    ...
    try {
        ...
        const linked = await legacy.query(`SELECT training_session_id AS "trainingSessionId" FROM conversation WHERE id = $1`, [CONVERSATION]);
        assert.ok(linked.rows[0]?.trainingSessionId, 'legacy conversation gains a session');
        const session = await legacy.query(`SELECT source_type AS "sourceType", learner_id AS "learnerId", assignment_id AS "assignmentId"
...
test('0015 keeps conversation.training_session_id nullable during the expand-and-contract transition', { timeout: 120_000 }, async () => {
    ...
    try {
        ...
        const seeded = await client.query(`SELECT training_session_id AS "sessionId" FROM conversation WHERE id = $1`, [CONVERSATION]);
        assert.equal(seeded.rows[0]?.sessionId, null, 'legacy path leaves session_id null until backfill/service writes it');
        const nullable = await client.query(`SELECT is_nullable AS "isNullable" FROM information_schema.columns
...
test('0015 enforces dual-track consistency: free rejects assigned chain and vice versa', { timeout: 120_000 }, async () => {
    ...
    const client = await applyAll(postgres.connectionUri);
    try {
        await client.query(`INSERT INTO learner_profile (internal_learner_id, organization_id, external_principal_id)
       VALUES ($1, $2, 'pgu-1')`, [LEARNER, ORG]);
        // Valid free session: no assigned chain columns at all.
        await assert.doesNotReject(client.query(`INSERT INTO training_session
           (id, organization_id, learner_id, source_type, persona_snapshot, status, idempotency_key)
         VALUES (gen_random_uuid(), $1, $2, 'free', '{}'::jsonb, 'ended', 'free-1')`, [ORG, LEARNER]));
        // Invalid free session: carries an assignment_id → CHECK rejects.
        await assert.rejects(client.query(`INSERT INTO training_session
           (id, organization_id, learner_id, source_type, assignment_id, persona_snapshot, status, idempotency_key)
         VALUES (gen_random_uuid(), $1, $2, 'free', $3, '{}'::jsonb, 'ended', 'free-2')`, [ORG, LEARNER, ASSIGNMENT]));
        // Invalid assigned session: declares assigned but omits the chain → CHECK rejects.
        await assert.rejects(client.query(`INSERT INTO training_session
           (id, organization_id, learner_id, source_type, persona_snapshot, status, idempotency_key)
         VALUES (gen_random_uuid(), $1, $2, 'assigned', '{}'::jsonb, 'ended', 'assigned-bad')`, [ORG, LEARNER]));
    }
    finally {
        await client.close();
        await postgres.stop();
...
test('0015 rejects cross-organization learner reference via composite foreign key', { timeout: 120_000 }, async () => {
    ...
    try {
        ...
        await client.query(`INSERT INTO learner_profile (internal_learner_id, organization_id, external_principal_id)
       VALUES ($1, $2, 'pgu-x')`, [LEARNER, ORG_OTHER]);
        // Session in ORG referencing that learner → composite FK rejects.
        await assert.rejects(client.query(`INSERT INTO training_session
           (id, organization_id, learner_id, source_type, persona_snapshot, status, idempotency_key)
         VALUES (gen_random_uuid(), $1, $2, 'free', '{}'::jsonb, 'active', 'cross-1')`, [ORG, LEARNER]));
    }
    finally {
        await client.close();
        await postgres.stop();
...
test('0015 allows only one in-flight session per learner', { timeout: 120_000 }, async () => {
    ...
    try {
        await client.query(`INSERT INTO learner_profile (internal_learner_id, organization_id, external_principal_id)
       VALUES ($1, $2, 'pgu-2')`, [LEARNER, ORG]);
        await client.query(`INSERT INTO training_session
         (id, organization_id, learner_id, source_type, persona_snapshot, status, idempotency_key)
       VALUES (gen_random_uuid(), $1, $2, 'free', '{}'::jsonb, 'active', 'a-1')`, [ORG, LEARNER]);
        // Second in-flight session for the same learner is rejected.
        await assert.rejects(client.query(`INSERT INTO training_session
           (id, organization_id, learner_id, source_type, persona_snapshot, status, idempotency_key)
         VALUES (gen_random_uuid(), $1, $2, 'free', '{}'::jsonb, 'created', 'a-2')`, [ORG, LEARNER]));
        // A finished session does not occupy the in-flight slot.
        await assert.doesNotReject(client.query(`INSERT INTO training_session
           (id, organization_id, learner_id, source_type, persona_snapshot, status, idempotency_key)
         VALUES (gen_random_uuid(), $1, $2, 'free', '{}'::jsonb, 'ended', 'a-3')`, [ORG, LEARNER]));
    }
    finally {
        await client.close();
        await postgres.stop();
...
test('0015 enforces personal template ownership and rejects cross-scope owner misuse', { timeout: 120_000 }, async () => {
    ...
    try {
        await client.query(`INSERT INTO learner_profile (internal_learner_id, organization_id, external_principal_id)
       VALUES ($1, $2, 'pgu-3')`, [LEARNER, ORG]);
        // personal without owner → reject
        await assert.rejects(client.query(`INSERT INTO training_template (id, organization_id, scope, title, persona_config)
         VALUES (gen_random_uuid(), $1, 'personal', 'mine', '{}'::jsonb)`, [ORG]));
        // organization with an owner → reject
        await assert.rejects(client.query(`INSERT INTO training_template (id, organization_id, scope, owner_learner_id, title, persona_config)
         VALUES (gen_random_uuid(), $1, 'organization', $2, 'org-tpl', '{}'::jsonb)`, [ORG, LEARNER]));
        // valid personal template
        await assert.doesNotReject(client.query(`INSERT INTO training_template (id, organization_id, scope, owner_learner_id, title, persona_config)
         VALUES (gen_random_uuid(), $1, 'personal', $2, 'mine', '{}'::jsonb)`, [ORG, LEARNER]));
    }
    finally {
        await client.close();
        await postgres.stop();
