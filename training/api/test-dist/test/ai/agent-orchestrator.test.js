...
import { createInitialCustomerState, ModelProviderError } from '@training/contracts';
...
test('knowledge belonging to another organization is rejected before the model is called', async () => {
    ...
    await assert.rejects(() => orchestrator.buildAndValidateContext({
        id: 'conversation-1',
        ...
        agentConfig: {},
    }, []), (error) => error instanceof OrchestratorError && error.code === 'KNOWLEDGE_NOT_AVAILABLE');
...
test('legacy model output receives a state fallback and an absolute turn number', async () => {
    ...
    assert.equal((model.lastRequest?.simulation).turnNumber, 25);
});
test('orchestrator maps a real provider timeout onto MODEL_TIMEOUT', async () => {
    const orchestrator = new AgentOrchestrator(new FakeKnowledgeAdapter(), new ThrowingModel(new ModelProviderError('MODEL_TIMEOUT', 'timed out')));
    await assert.rejects(() => orchestrator.generateDecision(minimalContext()), (error) => error instanceof OrchestratorError && error.code === 'MODEL_TIMEOUT');
});
test('orchestrator maps an upstream outage onto MODEL_UPSTREAM_UNAVAILABLE', async () => {
    const orchestrator = new AgentOrchestrator(new FakeKnowledgeAdapter(), new ThrowingModel(new ModelProviderError('MODEL_UPSTREAM_UNAVAILABLE', 'gateway 502')));
    await assert.rejects(() => orchestrator.generateDecision(minimalContext()), (error) => error instanceof OrchestratorError && error.code === 'MODEL_UPSTREAM_UNAVAILABLE');
});
test('orchestrator maps a malformed provider response onto MODEL_SCHEMA_INVALID', async () => {
    const orchestrator = new AgentOrchestrator(new FakeKnowledgeAdapter(), new ThrowingModel(new ModelProviderError('MODEL_RESPONSE_INVALID', 'bad shape')));
    await assert.rejects(() => orchestrator.generateDecision(minimalContext()), (error) => error instanceof OrchestratorError && error.code === 'MODEL_SCHEMA_INVALID');
});
