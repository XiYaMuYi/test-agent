...
async function boot(context) {
    ...
    async function insertLearner(organizationId, principalId, displayName, patch = {}) {
...
test('admin learner detail returns profile plus recent sessions and is isolated/admin-only', { timeout: 120_000 }, async (context) => {
    ...
    await db.query(`INSERT INTO assignment (id, organization_id, release_snapshot_id, name, status, starts_at, ends_at, max_attempts, target_principal_ids)
     VALUES ($1,$2,$3,$4,'ended',NOW()-INTERVAL '10 days',NOW()-INTERVAL '9 days',2,'[]'::jsonb)`, [assignmentId, ORGANIZATION_A, snapshotId, '9月抗老开场专项']);
    ...
    await db.query(`INSERT INTO learner_assignment (id, organization_id, assignment_id, learner_id, state, total_attempts)
     VALUES ($1,$2,$3,$4,'completed',1)`, [learnerAssignmentId, ORGANIZATION_A, assignmentId, learnerId]);
    await db.query(`INSERT INTO training_attempt (id, organization_id, learner_assignment_id, idempotency_key, status)
     VALUES ($1,$2,$3,'att-assigned','completed')`, [attemptId, ORGANIZATION_A, learnerAssignmentId]);
    await db.query(`INSERT INTO training_session (id, organization_id, learner_id, source_type, mode, release_snapshot_id, assignment_id, learner_assignment_id, training_attempt_id, persona_snapshot, status, idempotency_key, started_at, finished_at)
     VALUES ($1,$2,$3,'assigned','exam',$4,$5,$6,$7,'{}'::jsonb,'scored','key-assigned',NOW()-INTERVAL '2 days',NOW()-INTERVAL '2 days'+INTERVAL '10 minutes')`, [assignedSessionId, ORGANIZATION_A, learnerId, snapshotId, assignmentId, learnerAssignmentId, attemptId]);
    await db.query(`INSERT INTO training_session (id, organization_id, learner_id, source_type, mode, persona_snapshot, status, idempotency_key, started_at)
     VALUES ($1,$2,$3,'free','practice','{}'::jsonb,'ended','key-free',NOW()-INTERVAL '1 hour')`, [freeSessionId, ORGANIZATION_A, learnerId]);
