...
async function boot(context) {
    ...
    async function insertLearner(organizationId, principalId, displayName) {
        const learnerId = crypto.randomUUID();
        await db.query(`INSERT INTO learner_profile
       ...
       VALUES ($1,$2,$3,'gongzhugou',$4,0,0,'{}'::jsonb,'[]'::jsonb)`, [learnerId, organizationId, principalId, displayName]);
        return learnerId;
    }
    // assigned 训练链：场景→快照→任务→学员任务→尝试→会话
    async function insertAssignedSession(organizationId, learnerId, scenarioTitle, assignmentName) {
       ...
       VALUES ($1,$2,$3,$4,'ended',NOW()-INTERVAL '10 days',NOW()-INTERVAL '9 days',2,'[]'::jsonb)`, [assignmentId, organizationId, snapshotId, assignmentName]);
        await db.query(`INSERT INTO learner_assignment (id, organization_id, assignment_id, learner_id, state, total_attempts)
       VALUES ($1,$2,$3,$4,'completed',1)`, [learnerAssignmentId, organizationId, assignmentId, learnerId]);
        await db.query(`INSERT INTO training_attempt (id, organization_id, learner_assignment_id, idempotency_key, status)
       VALUES ($1,$2,$3,$4,'completed')`, [attemptId, organizationId, learnerAssignmentId, `att-${sessionId}`]);
        await db.query(`INSERT INTO training_session (id, organization_id, learner_id, source_type, mode, release_snapshot_id, assignment_id, learner_assignment_id, training_attempt_id, persona_snapshot, status, idempotency_key, started_at, finished_at)
       VALUES ($1,$2,$3,'assigned','exam',$4,$5,$6,$7,'{}'::jsonb,'scored',$8,NOW()-INTERVAL '2 days',NOW()-INTERVAL '2 days'+INTERVAL '10 minutes')`, [sessionId, organizationId, learnerId, snapshotId, assignmentId, learnerAssignmentId, attemptId, `sess-${sessionId}`]);
        return { sessionId, snapshotId, attemptId };
    }
    async function insertFreeSession(organizationId, learnerId, startedAgo = '1 hour') {
        const sessionId = crypto.randomUUID();
        await db.query(`INSERT INTO training_session (id, organization_id, learner_id, source_type, mode, persona_snapshot, status, idempotency_key, started_at)
       VALUES ($1,$2,$3,'free','practice','{}'::jsonb,'ended',$4,NOW()-INTERVAL '${startedAgo}')`, [sessionId, organizationId, learnerId, `free-${sessionId}`]);
        return sessionId;
    }
    async function insertConversation(organizationId, sessionId, chain) {
       ...
       VALUES ($1,$2,$3,$4,$5,'ended',1,0)`, [conversationId, organizationId, chain.attemptId, chain.snapshotId, sessionId]);
        return conversationId;
    }
    async function insertMessage(organizationId, conversationId, sequence, learnerText, replyText) {
        ...
        await db.query(`INSERT INTO conversation_message (id, organization_id, conversation_id, sequence, client_message_id, role, content, request_hash, response_hash)
       VALUES ($1,$2,$3,$4,$5,'learner',$6,$7,$8)`, [messageId, organizationId, conversationId, sequence, `cm-${messageId}`, learnerText, `req-${messageId}`, responseHash]);
    }
    async function insertReport(organizationId, conversationId, score, createdAt) {
        const jobId = crypto.randomUUID();
        const reportId = crypto.randomUUID();
        await db.query(`INSERT INTO evaluation_job (id, organization_id, conversation_id, status, attempt_count)
       VALUES ($1,$2,$3,'succeeded',1)`, [jobId, organizationId, conversationId]);
        await db.query(`INSERT INTO evaluation_report (id, organization_id, conversation_id, evaluation_job_id, status, report${createdAt ? ', created_at' : ''})
       VALUES ($1,$2,$3,$4,'published',$5::jsonb${createdAt ? ', $6' : ''})`, createdAt
...
test('admin evaluation list joins business fields, filters by source/learner, paginates and is isolated', { timeout: 120_000 }, async (context) => {
    ...
    const assignedRow = all.body.items.find((r) => r.conversationId === assignedConv);
...
test('admin conversation replay returns ordered turns and report, and is isolated/admin-only', { timeout: 120_000 }, async (context) => {
    ...
    assert.deepEqual(replay.body.messages.map((m) => [m.role, m.content]), [
        ['learner', '你好，我想了解抗老产品'],
