...
import { ruleBasedCoachFeedback, analyzeMoodQuick, detectEvents, } from '../../src/ai/coach/index.js';
/**
...
function deriveCustomerMood(moodValue) {
...
test('coach feedback: polite product-aware turn scores well', () => {
    const result = ruleBasedCoachFeedback({
        userMsg: '您好！我推荐您试试我们的维生素C产品，含有天然针叶樱桃提取物，效果非常好，很多客户反馈满意。',
...
test('detectEvents: picks up need_revealed and closing_signal', () => {
    const events = detectEvents('我想买一款适合敏感肌的产品，多少钱？', '你说得对，这个确实不错');
...
test('mood accumulation: multi-turn scenario stays consistent', () => {
    ...
    let moodValue = 0;
    const events1 = detectEvents('您好，我想看看护肤品', '太好了，我非常满意');
    ...
    const events2 = detectEvents('这个产品太贵了，算了', '太贵了，不要了');
    ...
    if (moodValue > 10)
        assert.equal(mood2, 'positive');
    else if (moodValue < -10)
        assert.equal(mood2, 'negative');
    else
        assert.equal(mood2, 'neutral');
});
test('end-to-end enrichment shape matches MessageResponse contract', () => {
    ...
    const customerMood = deriveCustomerMood(newMoodValue);
    const coachFeedback = {
