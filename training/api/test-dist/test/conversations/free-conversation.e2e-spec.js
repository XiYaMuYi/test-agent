...
test('free conversation is learner-owned, exchanges messages without an assigned chain, and frees the active slot on end', { timeout: 120_000 }, async (context) => {
    ...
    assert.ok(['positive', 'neutral', 'negative'].includes(message.body.customerMood), 'client still receives the customerMood label');
    ...
    const chainCounts = await db.query(`SELECT
       ...
       (SELECT status FROM training_session WHERE id = $2) AS "sessionStatus"`, [conversationId, started.body.sessionId]);
