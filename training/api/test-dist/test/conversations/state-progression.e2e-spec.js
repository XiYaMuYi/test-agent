...
test('automatic termination rolls back as one transaction and retries with one report job', { timeout: 120_000 }, async (context) => {
    ...
    const failed = await db.query(`SELECT c.status, m.response_hash AS response FROM conversation c JOIN conversation_message m ON m.conversation_id = c.id WHERE c.id = $1`, [id]);
    ...
    const jobs = await db.query('SELECT COUNT(*)::text AS count FROM evaluation_job WHERE conversation_id=$1', [id]);
    assert.equal(jobs.rows[0]?.count, '1');
    const sessions = await db.query('SELECT status FROM training_session WHERE id=$1', [created.body.trainingSessionId ?? created.body.sessionId]);
