...
type RowCount = Record<string, unknown> & { count: string };

async function applyCoreSchema(connectionUri: string) {
...
async function seedCoreRows(client: Awaited<ReturnType<typeof applyCoreSchema>>) {
...
test('core MVP migration creates expected tables', { timeout: 120_000 }, async () => {
  ...
  try {
    const result = await client.query<RowCount>(`
...
test('core MVP constraints reject cross-organization relations, immutable records, and duplicate active attempts', { timeout: 120_000 }, async () => {
  ...
  try {
    ...
    await assert.rejects(
      client.query(`INSERT INTO release_snapshot (id, scenario_draft_id, organization_id, schema_version, snapshot) VALUES ('00000000-0000-0000-0000-000000000021', '00000000-0000-0000-0000-000000000001', '00000000-0000-0000-0000-000000000099', 'release-snapshot/v1', '{}'::jsonb)`),
    );

    await assert.doesNotReject(
      client.query(`INSERT INTO release_snapshot (id, scenario_draft_id, organization_id, schema_version, snapshot) VALUES ('00000000-0000-0000-0000-000000000024', '00000000-0000-0000-0000-000000000001', '00000000-0000-0000-0000-000000000010', 'release-snapshot/v1', '{}'::jsonb)`),
    );

    await assert.rejects(
      client.query(`INSERT INTO conversation (id, organization_id, training_attempt_id, release_snapshot_id, status) VALUES ('00000000-0000-0000-0000-000000000022', '00000000-0000-0000-0000-000000000010', '00000000-0000-0000-0000-000000000005', '00000000-0000-0000-0000-000000000002', 'active')`),
    );

    await assert.rejects(
      client.query(`INSERT INTO evaluation_job (id, organization_id, conversation_id, status) VALUES ('00000000-0000-0000-0000-000000000023', '00000000-0000-0000-0000-000000000010', '00000000-0000-0000-0000-000000000006', 'queued')`),
    );

    await assert.rejects(
      client.query(`UPDATE release_snapshot SET snapshot = '{"changed":true}'::jsonb WHERE id = '00000000-0000-0000-0000-000000000002'`),
    );

    await assert.rejects(
      client.query(`UPDATE evaluation_report SET report = '{"changed":true}'::jsonb WHERE id = '00000000-0000-0000-0000-000000000009'`),
    );

    await assert.rejects(
      client.query(`DELETE FROM evaluation_report WHERE id = '00000000-0000-0000-0000-000000000009'`),
    );

    await assert.rejects(
      client.query(`INSERT INTO training_attempt (id, organization_id, learner_assignment_id, idempotency_key, status) VALUES ('00000000-0000-0000-0000-000000000011', '00000000-0000-0000-0000-000000000010', '00000000-0000-0000-0000-000000000004', 'idem-2', 'active')`),
    );

    await assert.rejects(
      client.query(`INSERT INTO conversation_message (id, organization_id, conversation_id, sequence, client_message_id, role, content, request_hash) VALUES ('00000000-0000-0000-0000-000000000012', '00000000-0000-0000-0000-000000000010', '00000000-0000-0000-0000-000000000006', 1, 'msg-2', 'learner', 'hello again', 'hash-2')`),
    );

    await assert.rejects(
      client.query(`INSERT INTO evaluation_report (id, organization_id, conversation_id, evaluation_job_id, status, report) VALUES ('00000000-0000-0000-0000-000000000013', '00000000-0000-0000-0000-000000000010', '00000000-0000-0000-0000-000000000006', '00000000-0000-0000-0000-000000000008', 'published', '{}'::jsonb)`),
    );
  } finally {
    await client.close();
    await postgres.stop();
...
test('worker lease migration backfills in-flight work with a grace lease and rolls back cleanly', { timeout: 120_000 }, async () => {
  ...
  try {
    ...
    await client.query(`
      ...
      WHERE id = '00000000-0000-0000-0000-000000000008'
    `);
    const legacyClaims = await client.query<{ source: string; claimToken: string | null; remainingSeconds: number }>(`
      SELECT 'event' AS source,
             claim_token::text AS "claimToken",
             EXTRACT(EPOCH FROM (lease_expires_at - clock_timestamp()))::float8 AS "remainingSeconds"
      FROM outbox_event
    ...
    `);
    assert.deepEqual(
      legacyClaims.rows.map(({ source, claimToken }) => ({ source, claimToken })),
      [
    ...
    const columns = await client.query<RowCount>(`
      SELECT COUNT(*)::text AS count
    ...
    const objects = await client.query<RowCount>(`
      SELECT (
