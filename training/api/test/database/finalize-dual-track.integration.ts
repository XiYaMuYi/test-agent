...
async function seedAssignedChain(client: ReturnType<typeof createPostgresExecutor>): Promise<void> {
...
async function nullability(client: ReturnType<typeof createPostgresExecutor>, column: string): Promise<string> {
  const result = await client.query<{ isNullable: string }>(
    `SELECT is_nullable AS "isNullable" FROM information_schema.columns
     WHERE table_schema='public' AND table_name='conversation' AND column_name=$1`,
    [column],
  );
...
test('0016 backfills conversations the legacy write path created during the transition window', { timeout: 120_000 }, async () => {
  ...
  const before = await client.query<{ sessionId: string | null }>(
  ...
  try {
    ...
    const linked = await client.query<{ sessionId: string | null }>(
      'SELECT training_session_id AS "sessionId" FROM conversation WHERE id=$1',
      [CONVERSATION],
    );
    assert.ok(linked.rows[0]?.sessionId, 'straggler conversation gains a session');

    const session = await client.query<{ sourceType: string; learnerId: string }>(
      `SELECT source_type AS "sourceType", learner_id AS "learnerId"
...
test('0016 aborts and stays nullable when an unlinkable orphan conversation remains', { timeout: 120_000 }, async () => {
  ...
  await client.query(
    `INSERT INTO conversation (id, organization_id, status) VALUES ($1, $2, 'created')`,
    [ORPHAN, ORG],
  );
