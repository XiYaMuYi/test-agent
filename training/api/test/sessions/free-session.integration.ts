...
interface SessionRow {
  ...
  learner_assignment_id: string | null;
  training_attempt_id: string | null;
  release_snapshot_id: string | null;
  template_id: string | null;
  persona_snapshot: { basedOnCard: string; conversation: { productScenario: string } };
...
test('startFreeSession creates a session + conversation + learner profile with no assignment chain and freezes persona', { timeout: 120_000 }, async () => {
  ...
  try {
    ...
    const session = await pool.query<SessionRow>(
      `SELECT source_type, learner_id, mode, status, assignment_id, learner_assignment_id,
    ...
    const conversation = await pool.query<{ training_session_id: string; training_attempt_id: string | null; release_snapshot_id: string | null; status: string }>(
      `SELECT training_session_id, training_attempt_id, release_snapshot_id, status
    ...
    const profile = await pool.query<{ internal_learner_id: string; external_principal_id: string; identity_provider: string }>(
      `SELECT internal_learner_id, external_principal_id, identity_provider FROM learner_profile
       WHERE organization_id = $1 AND external_principal_id = $2`,
      [ORG, PRINCIPAL_ID],
    );
...
test('startFreeSession replays the same idempotency key and blocks a second distinct session while one is active', { timeout: 120_000 }, async () => {
  ...
  try {
    ...
    await assert.rejects(
      () => sessions.startFreeSession(principal(), { persona: personaInput() }, 'different-key'),
...
test('startFreeSession allows another session after the previous ends, reuses the profile, and persists exam mode', { timeout: 120_000 }, async () => {
  ...
  try {
    ...
    const mode = await pool.query<{ mode: string }>(`SELECT mode FROM training_session WHERE id = $1`, [second.sessionId]);
...
test('startFreeSession starts from a visible template, records template_id, prefers inline persona, rejects hidden templates', { timeout: 120_000 }, async () => {
  ...
  try {
    ...
    const fromTemplateRow = await pool.query<{ template_id: string | null; persona_snapshot: { name: string } }>(
      'SELECT template_id, persona_snapshot FROM training_session WHERE id = $1',
      [fromTemplate.sessionId],
    );
    ...
    const bothRow = await pool.query<{ template_id: string | null; persona_snapshot: { basedOnCard: string } }>(
      'SELECT template_id, persona_snapshot FROM training_session WHERE id = $1',
      [both.sessionId],
    );
    ...
    const foreignTemplate = await templates.createPersonalTemplate(
      principal({ principalId: 'streamer-002' }),
      { title: '别人的模板', personaConfig: buildDefaultConfig() },
    );
    await assert.rejects(
      () => sessions.startFreeSession(principal(), { templateId: foreignTemplate.id }, 'hidden'),
      /FREE_SESSION_TEMPLATE_UNAVAILABLE/,
    );

    // Neither templateId nor persona supplied.
    await assert.rejects(
      () => sessions.startFreeSession(principal(), {}, 'neither'),
      /SCHEMA_INVALID/,
    );
  } finally {
    await pool.end();
