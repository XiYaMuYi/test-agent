...
import type { CurrentPrincipal } from '../../src/identity/identity-context.js';
...
interface SeedOptions {
  ...
  readonly snapshotPersona?: unknown;
}

async function seedAssignment(pool: Pool, options: SeedOptions = {}): Promise<{ assignmentId: string; snapshotId: string; laId: string }> {
  ...
  await pool.query(
    `INSERT INTO scenario_draft (id, organization_id, title, payload)
     VALUES ($1, $2, 'd', '{"title":"d","knowledgeVersions":["k@v1"],"scoringRules":["r1"],"agentConfig":{}}'::jsonb)`,
    [draftId, ORG],
  );
  ...
  await pool.query(
    `INSERT INTO release_snapshot (id, scenario_draft_id, organization_id, schema_version, snapshot)
     VALUES ($1, $2, $3, 'release-snapshot/v1', $4::jsonb)`,
    [snapshotId, draftId, ORG, JSON.stringify(snapshot)],
  );
  await pool.query(
  ...
  await pool.query(
    ...
    [laId, ORG, assignmentId, LEARNER, options.learnerState ?? 'eligible', options.totalAttempts ?? 0, options.activeAttemptId ?? null],
  );
  return { assignmentId, snapshotId, laId };
}

async function newService(uri: string): Promise<{ pool: Pool; sessions: SessionService }> {
...
test('startAssignedSession creates attempt + assigned session + conversation in one transaction and is idempotent', { timeout: 120_000 }, async () => {
  ...
  try {
    ...
    const session = await pool.query<{
      source_type: string; status: string; assignment_id: string; learner_assignment_id: string;
      training_attempt_id: string; release_snapshot_id: string; learner_id: string; persona_snapshot: Record<string, unknown>;
    }>(
      `SELECT source_type, status, assignment_id, learner_assignment_id, training_attempt_id,
    ...
    const conversation = await pool.query<{ training_attempt_id: string; release_snapshot_id: string; training_session_id: string }>(
      `SELECT training_attempt_id, release_snapshot_id, training_session_id FROM conversation WHERE id = $1`,
    ...
    const la = await pool.query<{ active_attempt_id: string; state: string }>(
      `SELECT active_attempt_id, state FROM learner_assignment WHERE id = $1`,
      [laId],
    );
...
test('an in-flight free session blocks an assigned start until it ends (global single-active across tracks)', { timeout: 120_000 }, async () => {
  ...
  try {
    ...
    await assert.rejects(
      () => sessions.startAssignedSession(principal(), assignmentId, 'assigned-while-free'),
      (error: unknown) =>
        error instanceof AssignmentProblem
        && (error.getResponse() as { code: string }).code === 'ATTEMPT_ALREADY_ACTIVE',
    );
...
test('startAssignedSession freezes the task persona from the release snapshot, defaulting when absent', { timeout: 120_000 }, async () => {
  ...
  try {
    ...
    const taskRow = await pool.query<{ persona_snapshot: { name: string } }>(
      'SELECT persona_snapshot FROM training_session WHERE id = $1',
      [first.sessionId],
    );
    ...
    const defaultRow = await pool.query<{ persona_snapshot: { basedOnCard: string } }>(
      'SELECT persona_snapshot FROM training_session WHERE id = $1',
      [second.sessionId],
    );
