...
test('rejects unknown top-level keys (contract drift guard)', () => {
  const config = { ...validConfig(), rogueField: 'x' } as unknown as PersonaConfig;
...
test('rejects unknown nested keys in each block', () => {
  const personality = { ...validConfig(), personality: { ...validConfig().personality, moody: 90 } } as unknown as PersonaConfig;
  assert.equal(service.validatePersonaConfig(personality).valid, false);

  const consumption = { ...validConfig(), consumption: { ...validConfig().consumption, fancy: 'x' } } as unknown as PersonaConfig;
...
test('rejects non-object nested blocks', () => {
  const badBasic = { ...validConfig(), basic: 'single' } as unknown as PersonaConfig;
