...
async function createFixture(context: test.TestContext) {
  ...
  await db.query(
    `INSERT INTO scenario_draft (id, organization_id, title, payload)
     VALUES ($1, $2, 'fixture draft', '{"title":"fixture draft","knowledgeVersions":["knowledge-welcome@v1"],"scoringRules":["r1"],"agentConfig":{}}'::jsonb)`,
    [crypto.randomUUID(), ORGANIZATION_A],
  );
  const draft = await db.query<{ id: string }>('SELECT id FROM scenario_draft WHERE organization_id = $1', [ORGANIZATION_A]);
  await db.query(
    `INSERT INTO release_snapshot (id, scenario_draft_id, organization_id, schema_version, snapshot)
     VALUES ($1, $2, $3, 'release-snapshot/v1', '{"schemaVersion":"release-snapshot/v1"}'::jsonb)`,
    [releaseSnapshotId, draft.rows[0]?.id, ORGANIZATION_A],
  );
  return { app, db, releaseSnapshotId };
}

function activeAssignmentBody(releaseSnapshotId: string, targetPrincipalIds: readonly string[], overrides: Record<string, unknown> = {}) {
  return {
...
test('duplicate authorized targets merge to one learner assignment and streamer sees only own task', { timeout: 120_000 }, async (context) => {
  ...
  const learners = await db.query<{ learner_id: string }>('SELECT learner_id FROM learner_assignment');
...
test('starting an assignment atomically creates a conversation bound to its attempt and release snapshot', { timeout: 120_000 }, async (context) => {
  ...
  const conversation = await db.query<{ training_attempt_id: string; release_snapshot_id: string; organization_id: string }>(
    'SELECT training_attempt_id, release_snapshot_id, organization_id FROM conversation WHERE id = $1',
...
test('conversation creation failure rolls back its attempt atomically', { timeout: 120_000 }, async (context) => {
  ...
  const attempts = await db.query<{ count: string }>('SELECT COUNT(*)::text AS count FROM training_attempt WHERE organization_id = $1', [ORGANIZATION_A]);
