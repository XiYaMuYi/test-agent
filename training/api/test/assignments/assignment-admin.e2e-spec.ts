...
async function boot(context: test.TestContext, draftTitle = '抗老咨询场景') {
  ...
  await db.query(
    `INSERT INTO scenario_draft (id, organization_id, title, payload)
     VALUES ($1, $2, $3, '{}'::jsonb)`,
    [draftId, ORGANIZATION_A, draftTitle],
  );
  await db.query(
    ...
    [releaseSnapshotId, draftId, ORGANIZATION_A],
  );
  return { app, db, releaseSnapshotId };
}

function body(releaseSnapshotId: string, targets: readonly string[], overrides: Record<string, unknown> = {}) {
  return {
    releaseSnapshotId,
    name: '新品抗老陪练任务',
    status: 'active',
...
test('assignment id is server-generated, admin list joins scenario title and aggregates learners, detail returns roster', { timeout: 120_000 }, async (context) => {
  ...
  const assignmentId: string = withoutId.body.assignmentId;
  ...
  const first = list.body.items.find((item: { id: string }) => item.id === assignmentId);
  ...
  assert.deepEqual(pausedOnly.body.items.map((i: { id: string }) => i.id), [explicitId]);
  ...
  const states = detail.body.learners.map((l: { state: string }) => l.state).sort();
...
test('assignment status machine enforces legal transitions and is isolated/admin-only', { timeout: 120_000 }, async (context) => {
  ...
  const pause = async (status: string) => request(http).post(`/admin/assignments/${id}/status`).set('x-access-token', ADMIN_A).send({ status });
...
test('task-level parameter override: create with patch, update it, reject invalid & ended', { timeout: 120_000 }, async (context) => {
  ...
  const list = await request(http).get('/admin/assignments').set('x-access-token', ADMIN_A).expect(200);
  const row = list.body.items.find((item: { id: string }) => item.id === id);
...
test('task-level override is frozen into a newly started assigned session; later edits do not touch it', { timeout: 120_000 }, async (context) => {
  ...
  const firstSession = (await db.query<{ training_session_id: string }>(
    `SELECT training_session_id FROM conversation WHERE id = $1`, [first.body.conversationId],
  )).rows[0]?.training_session_id;
  assert.ok(firstSession !== undefined, 'conversation must be bound to a training_session');

  const frozen = await db.query<{ persona: unknown; agent: unknown }>(
    `SELECT persona_snapshot AS persona, agent_config AS agent FROM training_session WHERE id = $1`,
    [firstSession],
  );
  const row = frozen.rows[0];
  assert.ok(row !== undefined, 'training_session must exist');
  const persona = row.persona as { conversation: { maxTurns: number; openingMode: string; background: string } };
  const agent = row.agent as { historyMessageLimit: number; closingTendency: string; responseLength: string };
  ...
  const second = await request(http).post(`/me/assignments/${id}/attempts`)
    ...
    .expect(201);
  const secondSession = (await db.query<{ training_session_id: string }>(
    `SELECT training_session_id FROM conversation WHERE id = $1`, [second.body.conversationId],
  )).rows[0]?.training_session_id;
  assert.ok(secondSession !== undefined, 'second conversation must be bound to a training_session');
  const secondRow = (await db.query<{ persona: unknown }>(
    `SELECT persona_snapshot AS persona FROM training_session WHERE id = $1`, [secondSession],
  )).rows[0];
  assert.ok(secondRow !== undefined, 'second training_session must exist');
  const secondPersona = secondRow.persona as { conversation: { maxTurns: number } };
  assert.equal(secondPersona.conversation.maxTurns, 12, 'new session picks up the updated override');
  const firstRowAfter = (await db.query<{ persona: unknown }>(
    `SELECT persona_snapshot AS persona FROM training_session WHERE id = $1`, [firstSession],
  )).rows[0];
  assert.ok(firstRowAfter !== undefined, 'first training_session must still exist');
  const firstPersonaAfter = firstRowAfter.persona as { conversation: { maxTurns: number } };
