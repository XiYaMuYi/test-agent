...
import { PersonaService, type BuildPersonaInput } from '../../src/persona/persona.service.js';
import {
  CUSTOMER_RELATIONS,
  CUSTOMER_COHORTS,
  TRUST_LEVELS,
...
const BASE_INPUT: BuildPersonaInput = {
  ageCardId: 'light-mature',
  difficulty: 2,
  productScenarioId: 'whitening',
};

function validConfig(): PersonaConfig {
...
test('product-scenario categories preserve all legacy 8 flat ids', () => {
  ...
  for (const legacyId of legacyIds) {
    const categoryId = PRODUCT_SCENARIO_LEGACY_CATEGORY_MAP[legacyId as keyof typeof PRODUCT_SCENARIO_LEGACY_CATEGORY_MAP];
    assert.ok(categoryId !== undefined, `legacy scenario ${legacyId} has no category mapping`);
    assert.ok(
      PRODUCT_SCENARIO_CATEGORIES.some((category) => category.id === categoryId),
      `category ${categoryId} of ${legacyId} must exist`,
    );
  }
...
test('field metadata exposes the new v2 dictionaries', () => {
  const meta = service.getFieldMetadata() as Record<string, any>;
  const basic = meta.basic as Record<string, unknown>;
  assert.equal((basic.customerRelations as readonly unknown[]).length, 5);
  assert.equal((basic.customerCohorts as readonly unknown[]).length, 8);
  assert.equal((basic.trustLevels as readonly unknown[]).length, 5);
  assert.equal(basic.cityMaxLength, 30);
  assert.equal(basic.purchaseCategoryMaxLength, 50);

  const consumption = meta.consumption as Record<string, unknown>;
  assert.equal((consumption.skinTypes as readonly { value: string }[]).some((s) => s.value === 'mixed_dry'), true);
  assert.equal((consumption.skinConcernsDictionary as readonly unknown[]).length, 6);

  const presets = meta.presets as Record<string, unknown>;
  assert.equal((presets.productScenarioCategories as readonly unknown[]).length, 5);
});
...
test('buildPersonaConfig rejects unknown scenario id', () => {
  assert.throws(
    () => service.buildPersonaConfig({ ...BASE_INPUT, productScenarioId: 'children::不存在的场景' }),
    (error: unknown) => (error as Error).message === 'PERSONA_PRESET_NOT_FOUND',
  );
});
