...
async function boot(context: { after: (fn: () => Promise<unknown>) => void }) {
...
test('admin scenario lifecycle: server-generated id, draft list/detail, published counts and snapshot list', { timeout: 120_000 }, async (context) => {
  ...
  const first = list.body.items.find((item: { id: string }) => item.id === draftId);
  ...
  const firstAfter = listAfter.body.items.find((item: { id: string }) => item.id === draftId);
