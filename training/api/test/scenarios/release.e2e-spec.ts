...
test('publish flow creates snapshot and outbox and preserves immutability', { timeout: 120_000 }, async (context) => {
  ...
  const snapshotRows = await db.query<{ id: string; organization_id: string; scenario_draft_id: string; snapshot: Record<string, unknown> }>(
    'SELECT id, organization_id, scenario_draft_id, snapshot FROM release_snapshot ORDER BY created_at',
  );
  ...
  const outboxRows = await db.query<{ event_type: string; status: string }>(
    'SELECT event_type, status FROM outbox_event ORDER BY created_at',
  );
  ...
  const snapshotRowsAfter = await db.query<{ id: string }>('SELECT id FROM release_snapshot ORDER BY created_at');
  assert.equal(snapshotRowsAfter.rows.length, 2);
  assert.notEqual(snapshotRowsAfter.rows[0]?.id, snapshotRowsAfter.rows[1]?.id);

  const outboxRowsAfter = await db.query<{ id: string }>('SELECT id FROM outbox_event ORDER BY created_at');
