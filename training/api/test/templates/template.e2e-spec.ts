...
test('platform/organization templates are visible to learners while admin maintenance is role-gated', { timeout: 120_000 }, async (context) => {
  ...
  await db.query(
    ...
    [PLATFORM_TEMPLATE, PLATFORM_ORG, JSON.stringify(buildDefaultConfig())],
  );
  context.after(async () => {
    await app.close();
    await db.close();
  ...
  const visible = await request(app.getHttpServer())
    ...
    .set('x-access-token', STREAMER)
    .expect(200);
  const scopes = visible.body.items.map((item: { scope: string }) => item.scope).sort();
  ...
  assert.deepEqual(
    ...
    [orgTemplate.body.id],
    'admin maintenance list shows only organization templates',
  );
});
