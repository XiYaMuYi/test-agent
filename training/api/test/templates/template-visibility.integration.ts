...
function principal(principalId: string, organizationId: string, roles: ReadonlyArray<'admin' | 'streamer'> = ['streamer']): CurrentPrincipal {
...
async function seedTemplate(
  ...
  scope: 'platform' | 'organization' | 'personal',
  ownerLearnerId: string | null,
  title: string,
  status: 'active' | 'archived' = 'active',
): Promise<void> {
  await pool.query(
    `INSERT INTO training_template (id, organization_id, scope, owner_learner_id, title, persona_config, status)
     VALUES ($1, $2, $3, $4, $5, $6::jsonb, $7)`,
    [id, organizationId, scope, ownerLearnerId, title, JSON.stringify(buildDefaultConfig()), status],
  );
}

async function prepareDatabase(uri: string): Promise<Pool> {
  ...
  for (const [learner, org, external] of [
    ...
    await pool.query(
      `INSERT INTO learner_profile (internal_learner_id, organization_id, external_principal_id, identity_provider)
       VALUES ($1, $2, $3, 'gongzhugou') ON CONFLICT DO NOTHING`,
      [learner, org, external],
    );
  }
...
test('learner sees platform + own-organization + own-personal templates only', { timeout: 120_000 }, async () => {
  ...
  try {
    ...
    assert.deepEqual(
      a1.map((item) => item.id).sort(),
      [T_ORG_A, T_PERSONAL_A1, T_PLATFORM].sort(),
      'learner A1 sees platform, org A and only their own personal template',
    );

    const a2 = await service.listVisibleTemplates(principal('streamer-002', ORG_A));
    assert.deepEqual(
      a2.map((item) => item.id).sort(),
      [T_ORG_A, T_PERSONAL_A2, T_PLATFORM].sort(),
      'learner A2 cannot see A1 personal template',
    );

    const b = await service.listVisibleTemplates(principal('streamer-b-001', ORG_B));
    assert.deepEqual(
      b.map((item) => item.id).sort(),
      [T_ORG_B, T_PLATFORM].sort(),
      'org B learner cannot see org A templates',
    );
  } finally {
    await pool.end();
    await postgres.stop();
  ...
  try {
    ...
    for (const hidden of [T_PERSONAL_A2, T_ORG_B, T_PLATFORM_ARCHIVED, T_MISSING]) {
      await assert.rejects(
        () => service.resolveVisibleTemplate(principal('streamer-001', ORG_A), hidden),
...
test('createPersonalTemplate persists an owned personal template and rejects invalid persona or duplicate title', { timeout: 120_000 }, async () => {
  ...
  try {
    ...
    const persisted = await pool.query<{ scope: string; owner_learner_id: string | null }>(
      'SELECT scope, owner_learner_id FROM training_template WHERE id = $1',
      [created.id],
    );
    ...
    await assert.rejects(
      () => service.createPersonalTemplate(principal('streamer-001', ORG_A), { title: 'bad', personaConfig: { not: 'a persona' } }),
      /PERSONA_CONFIG_INVALID/,
    );
    await assert.rejects(
      () => service.createPersonalTemplate(principal('streamer-001', ORG_A), { title: '我保存的画像', personaConfig: buildDefaultConfig() }),
