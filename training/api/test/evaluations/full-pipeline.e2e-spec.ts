...
interface EvaluationWorkerFixture {
  pollOnce(): Promise<{ scanned: number; dispatched: number; succeeded: number; failed: number }>;
}

async function createEvaluationWorker(database: TransactionalEvaluationExecutorPort): Promise<EvaluationWorkerFixture> {
  ...
  const Generator = generatorModule.RuleBasedEvaluationReportGenerator as new () => {
    generate(input: unknown): Promise<Record<string, unknown>>;
  };
  const createWorkerModule = workerModule.createWorkerModule as (options: {
    evaluationExecutor: TransactionalEvaluationExecutorPort;
    evaluationGenerator: { generate(input: unknown): Promise<Record<string, unknown>> };
  }) => EvaluationWorkerFixture;
...
async function waitForPublishedReport(
  ...
  while (Date.now() - started < timeoutMs) {
    const { rows } = await db.query<{ status: string }>(
      'SELECT status FROM outbox_event WHERE aggregate_id = $1 AND event_type = $2',
      [conversationId, 'evaluation.requested'],
    );
    if (rows[0]?.status === 'published') return;
    if (rows[0]?.status === 'failed') throw new Error('outbox_event reached failed status');
...
test('full pipeline: free session -> messages -> end -> published report with five-dimension scores and learner profile write-back', { timeout: 180_000 }, async (context) => {
  ...
  const { rows: jobsAfterEnd } = await db.query<{ id: string; status: string }>(
    'SELECT id, status FROM evaluation_job WHERE conversation_id = $1',
    [conversationId],
  );
  assert.equal(jobsAfterEnd.length, 1, 'evaluation_job must be created on end');

  const { rows: outboxAfterEnd } = await db.query<{ id: string; status: string }>(
    "SELECT id, status FROM outbox_event WHERE aggregate_id = $1 AND event_type = 'evaluation.requested'",
    [conversationId],
  );
  ...
  for (let iterations = 0; iterations < 30; iterations += 1) {
    ...
    if (rows[0]?.status === 'published') break;
    if (rows[0]?.status === 'failed') throw new Error('evaluation outbox reached failed status');
  }
  ...
  const { rows: reportRows } = await db.query<{
    ...
    'SELECT id, status, report FROM evaluation_report WHERE conversation_id = $1',
    [conversationId],
  );
  ...
  for (const dim of expectedDimensions) {
    ...
    const value = (body.dimensionScores as Record<string, number>)[dim] as number;
  ...
  assert.ok((body.highlights as unknown[]).length > 0 || (body.improvements as unknown[]).length > 0,
    'at least one of highlights/improvements must be non-empty');
  const chineseRegex = /[一-龥]/;
  for (const item of body.highlights as string[]) {
    assert.equal(typeof item, 'string');
    assert.match(item, chineseRegex, `highlight "${item}" must contain Chinese characters`);
  }
  for (const item of body.improvements as string[]) {
    assert.equal(typeof item, 'string');
  ...
  const { rows: profileRows } = await db.query<{
    ...
    dimension_scores: Record<string, { score: number; samples: number }>;
    weak_points: string[];
  }>(
    `SELECT total_free_sessions, total_assigned_sessions, avg_score::text AS avg_score,
  ...
  for (const dim of expectedDimensions) {
    const entry: { score: number; samples: number } = profile.dimension_scores[dim] as { score: number; samples: number };
