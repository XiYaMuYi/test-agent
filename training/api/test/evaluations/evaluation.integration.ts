...
interface EvaluationGeneratorFixture {
  generate(input: Record<string, unknown>): Promise<Record<string, unknown>>;
}

async function createEvaluationWorker(
  database: TransactionalEvaluationExecutorPort,
  options: {
    readonly generator?: EvaluationGeneratorFixture;
    readonly leaseDurationMs?: number;
    readonly now?: () => Date;
  } = {},
) {
  const loadWorkerModule = new Function('path', 'return import(path)') as (path: string) => Promise<{
    createWorkerModule(options: {
      evaluationExecutor: typeof database;
      evaluationGenerator?: EvaluationGeneratorFixture;
      evaluationLeaseDurationMs?: number;
      now?: () => Date;
...
function interceptTransactionQueries(
  database: TransactionalEvaluationExecutorPort,
...
async function createRuntimeEvaluationWorker(databaseUrl: string) {
  const loadWorkerMain = new Function('path', 'return import(path)') as (path: string) => Promise<{
    bootstrapWorker(): Promise<{
...
async function seedEvaluationFixture(dbUrl: string) {
  const db = createPostgresExecutor(dbUrl);
  await db.query(
    `INSERT INTO scenario_draft (id, organization_id, title, payload)
     VALUES ('11111111-1111-1111-1111-111111111110', $1, 'draft',
       '{"title":"draft","knowledgeVersions":["knowledge-welcome@v1"],"scoringRules":["rule-a"],"agentConfig":{}}'::jsonb)`,
    [ORG_A],
  );
  await db.query(
    `INSERT INTO release_snapshot (id, scenario_draft_id, organization_id, schema_version, snapshot)
     VALUES ('11111111-1111-1111-1111-111111111120', '11111111-1111-1111-1111-111111111110', $1, 'release-snapshot/v1',
       '{"schemaVersion":"release-snapshot/v1","releaseSnapshotId":"11111111-1111-1111-1111-111111111120","scenarioDraftId":"11111111-1111-1111-1111-111111111110","compiledAt":"2026-07-28T00:00:00.000Z","title":"draft","knowledgeVersions":["knowledge-welcome@v1"],"scoringRules":["rule-a"],"agentConfig":{}}'::jsonb)`,
    [ORG_A],
  );
  await db.query(
    `INSERT INTO assignment (id, organization_id, release_snapshot_id, name, status, starts_at, ends_at, max_attempts, target_principal_ids)
     VALUES ('11111111-1111-1111-1111-111111111130', $1, '11111111-1111-1111-1111-111111111120', 'assign', 'active',
       CURRENT_TIMESTAMP, CURRENT_TIMESTAMP + INTERVAL '1 day', 3, '[]'::jsonb)`,
    [ORG_A],
  );
  await db.query(
    `INSERT INTO learner_assignment (id, organization_id, assignment_id, learner_id, state, total_attempts, active_attempt_id)
     VALUES ('11111111-1111-1111-1111-111111111140', $1, '11111111-1111-1111-1111-111111111130', $2, 'active', 0, '11111111-1111-1111-1111-111111111150')`,
    [ORG_A, deriveLearnerId('streamer-001')],
  );
  await db.query(
    `INSERT INTO training_attempt (id, organization_id, learner_assignment_id, idempotency_key, status)
     VALUES ('11111111-1111-1111-1111-111111111150', $1, '11111111-1111-1111-1111-111111111140', 'start-1', 'created')`,
    [ORG_A],
  );
  await db.query(
    `INSERT INTO learner_profile (internal_learner_id, organization_id, external_principal_id, identity_provider)
     VALUES ($2, $1, 'streamer-001', 'gongzhugou') ON CONFLICT DO NOTHING`,
    [ORG_A, deriveLearnerId('streamer-001')],
  );
  await db.query(
    `INSERT INTO training_session (id, organization_id, learner_id, source_type, mode, release_snapshot_id, assignment_id, learner_assignment_id, training_attempt_id, persona_snapshot, status, idempotency_key)
  ...
  await db.query(
    `INSERT INTO conversation (id, organization_id, training_attempt_id, release_snapshot_id, training_session_id, status, version, last_sequence)
     VALUES ($1, $2, '11111111-1111-1111-1111-111111111150', '11111111-1111-1111-1111-111111111120', '11111111-1111-1111-1111-111111111171', 'ended', 2, 1)`,
    [CONV_ID, ORG_A],
  );
  await db.query(
    `INSERT INTO conversation_message (id, organization_id, conversation_id, sequence, client_message_id, role, content, request_hash)
     VALUES ('11111111-1111-1111-1111-111111111190', $1, $2, 1, 'msg-1', 'learner', 'hello', 'hash1')`,
    [ORG_A, CONV_ID],
  );
  await db.query(
    `INSERT INTO evaluation_job (id, organization_id, conversation_id, status, attempt_count)
     VALUES ($1, $2, $3, 'queued', 0)`,
    [JOB_ID, ORG_A, CONV_ID],
  );
  await db.query(
    ...
    [OUTBOX_ID, ORG_A, CONV_ID, JSON.stringify({ conversationId: CONV_ID, jobId: JOB_ID }), `evaluation:${CONV_ID}`],
  );
  await db.close();
}
...
test('worker idempotency: duplicate outbox delivery generates only one report', { timeout: 120_000 }, async (context) => {
  ...
  const { rows: reports } = await db.query<{ count: string }>(
  ...
  const { rows: reportsAfter } = await db.query<{ count: string }>(
    'SELECT COUNT(*)::text AS count FROM evaluation_report WHERE conversation_id = $1',
    [CONV_ID],
  );
...
test('an expired evaluation claim is reclaimed and the abandoned owner cannot publish a second report', { timeout: 120_000 }, async (context) => {
  ...
  while (firstCalls === 0) await new Promise((resolve) => setTimeout(resolve, 5));

  const claimedState = await db.query<{ eventStatus: string; jobStatus: string }>(
    `SELECT event.status AS "eventStatus", job.status AS "jobStatus"
  ...
  const finalState = await db.query<{
    ...
    `SELECT event.status AS "eventStatus",
     ...
     WHERE event.id = $2 AND event.organization_id = $3
     GROUP BY event.status, job.status, job.attempt_count`,
    [JOB_ID, OUTBOX_ID, ORG_A],
  );
...
test('an event reclaimed before its job lease expires remains recoverable until the job can be reclaimed', { timeout: 120_000 }, async (context) => {
  ...
  const { rows: deferredRows } = await db.query<{
    eventStatus: string;
    jobStatus: string;
    reportCount: string;
  }>(
    `SELECT event.status AS "eventStatus",
     ...
     LEFT JOIN evaluation_report report ON report.evaluation_job_id = job.id
       AND report.organization_id = job.organization_id
     WHERE event.id = $2 AND event.organization_id = $3
     GROUP BY event.status, job.status`,
    [JOB_ID, OUTBOX_ID, ORG_A],
  );
  assert.deepEqual(
    {
  ...
  assert.deepEqual(
    ...
    { scanned: 1, dispatched: 1, succeeded: 1, failed: 0 },
  );
...
test('a recovery claim fences an abandoned owner before either worker can persist a report', { timeout: 120_000 }, async (context) => {
  ...
  let announceRecoveryClaim!: () => void;
  const recoveryClaimReached = new Promise<void>((resolve) => { announceRecoveryClaim = resolve; });
  let releaseRecoveryClaim!: () => void;
  const recoveryClaimGate = new Promise<void>((resolve) => { releaseRecoveryClaim = resolve; });
  let announceAbandonedPersistence!: () => void;
  const abandonedPersistenceReached = new Promise<void>((resolve) => { announceAbandonedPersistence = resolve; });
  ...
  while (recoveryCalls === 0) await new Promise((resolve) => setTimeout(resolve, 5));
  ...
  const { rows } = await db.query<{ count: string; owner: string }>(
    `SELECT COUNT(*)::text AS count, MAX(report->>'owner') AS owner
...
test('an expired claim at the attempt limit becomes terminal without another generator call', { timeout: 120_000 }, async (context) => {
  ...
  const { rows: jobRows } = await db.query<{ status: string; attemptCount: string }>(
    'SELECT status, attempt_count::text AS "attemptCount" FROM evaluation_job WHERE id = $1 AND organization_id = $2',
    [JOB_ID, ORG_A],
  );
  assert.deepEqual(jobRows[0], { status: 'failed', attemptCount: '3' });
  const { rows: eventRows } = await db.query<{ status: string }>(
    'SELECT status FROM outbox_event WHERE id = $1 AND organization_id = $2',
    [OUTBOX_ID, ORG_A],
  );
...
test('two workers do not invoke the generator twice for duplicate evaluation events', { timeout: 120_000 }, async (context) => {
  ...
  await db.query(
    `INSERT INTO outbox_event (id, organization_id, aggregate_type, aggregate_id, event_type, payload, deduplication_key, status)
     VALUES ($1, $2, 'evaluation', $3, 'evaluation.requested', $4::jsonb, $5, 'pending')`,
    [duplicateEventId, ORG_A, CONV_ID, JSON.stringify({ conversationId: CONV_ID, jobId: JOB_ID }), `evaluation:${CONV_ID}:duplicate`],
  );
  let calls = 0;
  let release!: () => void;
  ...
  assert.equal(reports[0]?.count, '1');
  const { rows: duplicateEvents } = await db.query<{ status: string }>('SELECT status FROM outbox_event WHERE id = $1', [duplicateEventId]);
...
test('same conversation cannot produce two evaluation reports', { timeout: 120_000 }, async (context) => {
  ...
  await db.query(
    `INSERT INTO outbox_event (id, organization_id, aggregate_type, aggregate_id, event_type, payload, deduplication_key, status)
     VALUES ($1, $2, 'evaluation', $3, 'evaluation.requested', $4::jsonb, $5, 'pending')`,
    [outboxId2, ORG_A, CONV_ID, JSON.stringify({ conversationId: CONV_ID, jobId: JOB_ID }), `evaluation:${CONV_ID}:2`],
  );
...
test('published evaluation_report cannot be updated or deleted (DB constraint)', { timeout: 120_000 }, async (context) => {
  ...
  const { rows: reportRows } = await db.query<{ id: string }>(
    'SELECT id FROM evaluation_report WHERE conversation_id = $1',
    [CONV_ID],
  );
...
test('worker retries a transient evaluator failure up to its successful delivery', { timeout: 120_000 }, async (context) => {
  ...
  await consumer.pollOnce();
  let jobRows = await db.query<{ status: string; attemptCount: string }>(
    'SELECT status, attempt_count::text AS "attemptCount" FROM evaluation_job WHERE id = $1',
    [JOB_ID],
  );
  assert.deepEqual(jobRows.rows[0], { status: 'retryable_failed', attemptCount: '1' });

  await consumer.pollOnce();
  jobRows = await db.query<{ status: string; attemptCount: string }>(
    'SELECT status, attempt_count::text AS "attemptCount" FROM evaluation_job WHERE id = $1',
    [JOB_ID],
  );
  ...
  const { rows: completedJobRows } = await db.query<{ status: string; attemptCount: string }>(
    'SELECT status, attempt_count::text AS "attemptCount" FROM evaluation_job WHERE id = $1',
    [JOB_ID],
  );
...
test('worker terminally fails an always-retryable evaluator after the retry limit', { timeout: 120_000 }, async (context) => {
  ...
  assert.deepEqual(jobRows[0], { status: 'failed', attemptCount: '3' });
  const { rows: eventRows } = await db.query<{ status: string }>('SELECT status FROM outbox_event WHERE id = $1', [OUTBOX_ID]);
  assert.equal(eventRows[0]?.status, 'failed');
...
test('admin can only read evaluations from own organization', { timeout: 120_000 }, async (context) => {
  ...
  const orgAConversationIds = new Set<string>(adminA.body.items.map((e: { conversationId: string }) => e.conversationId));
  ...
  const orgBSeesOrgA = adminB.body.items.some((e: { conversationId: string }) => orgAConversationIds.has(e.conversationId));
...
test('conversation end triggers evaluation_job and outbox_event creation', { timeout: 120_000 }, async (context) => {
  ...
  await db.query(
    `INSERT INTO learner_assignment (id, organization_id, assignment_id, learner_id, state, total_attempts, active_attempt_id)
     VALUES ($1, $2, '11111111-1111-1111-1111-111111111130', $3, 'active', 0, $4)`,
    [LEARNER_ASSIGNMENT_NEW, ORG_A, deriveLearnerId('streamer-002'), ATTEMPT_NEW],
  );
  await db.query(
    `INSERT INTO learner_profile (internal_learner_id, organization_id, external_principal_id, identity_provider)
     VALUES ($1, $2, 'streamer-002', 'gongzhugou') ON CONFLICT DO NOTHING`,
    [deriveLearnerId('streamer-002'), ORG_A],
  );
  await db.query(
    `INSERT INTO training_attempt (id, organization_id, learner_assignment_id, idempotency_key, status)
     VALUES ($1, $2, $3, 'start-new', 'created')`,
    [ATTEMPT_NEW, ORG_A, LEARNER_ASSIGNMENT_NEW],
  );
  await db.query(
    `INSERT INTO training_session (id, organization_id, learner_id, source_type, mode, release_snapshot_id, assignment_id, learner_assignment_id, training_attempt_id, persona_snapshot, status, idempotency_key)
     VALUES ($1, $2, $3, 'assigned', 'practice', '11111111-1111-1111-1111-111111111120', '11111111-1111-1111-1111-111111111130', $4, $5, '{}'::jsonb, 'active', 'end-eval-session')`,
    [SESSION_NEW, ORG_A, deriveLearnerId('streamer-002'), LEARNER_ASSIGNMENT_NEW, ATTEMPT_NEW],
  );
  await db.query(
    `INSERT INTO conversation (id, organization_id, training_attempt_id, release_snapshot_id, training_session_id, status, version, last_sequence)
     VALUES ($1, $2, $3, '11111111-1111-1111-1111-111111111120', $4, 'active', 1, 0)`,
    [CONV_ID_NEW, ORG_A, ATTEMPT_NEW, SESSION_NEW],
  );
  ...
  const { rows: jobs } = await db.query<{ id: string }>(
    'SELECT id FROM evaluation_job WHERE conversation_id = $1',
    [CONV_ID_NEW],
  );
  assert.equal(jobs.length, 1, 'evaluation_job should be created');

  const { rows: outbox } = await db.query<{ id: string; event_type: string; status: string }>(
    "SELECT id, event_type AS \"eventType\", status FROM outbox_event WHERE aggregate_id = $1 AND event_type = 'evaluation.requested'",
    [CONV_ID_NEW],
  );
...
test('learner can read a free-session evaluation report through the session root, others cannot', { timeout: 120_000 }, async (context) => {
  ...
  await db.query(
    `INSERT INTO training_session (id, organization_id, learner_id, source_type, mode, persona_snapshot, status, idempotency_key)
     VALUES ($1, $2, $3, 'free', 'practice', '{}'::jsonb, 'ended', 'free-report-session')`,
    [SESSION_FREE, ORG_A, deriveLearnerId('streamer-001')],
  );
  await db.query(
    `INSERT INTO conversation (id, organization_id, training_session_id, status, version, last_sequence)
     VALUES ($1, $2, $3, 'ended', 1, 1)`,
    [CONV_FREE, ORG_A, SESSION_FREE],
  );
  await db.query(
    `INSERT INTO evaluation_job (id, organization_id, conversation_id, status, attempt_count)
     VALUES ($1, $2, $3, 'succeeded', 1)`,
    [JOB_FREE, ORG_A, CONV_FREE],
  );
  await db.query(
    `INSERT INTO evaluation_report (id, organization_id, conversation_id, evaluation_job_id, status, report)
     VALUES ($1, $2, $3, $4, 'published', '{"schemaVersion":"evaluation-report/v1","score":88}'::jsonb)`,
    [REPORT_FREE, ORG_A, CONV_FREE, JOB_FREE],
  );
...
async function seedScoringChain(
  dbUrl: string,
  kind: 'free' | 'assigned',
  ids: { readonly conversation: string; readonly session: string; readonly job: string; readonly event: string; readonly learnerAssignment?: string; readonly attempt?: string },
) {
  const db = createPostgresExecutor(dbUrl);
  ...
  if (kind === 'free') {
    await db.query(
      `INSERT INTO training_session (id, organization_id, learner_id, source_type, mode, persona_snapshot, status, idempotency_key)
       VALUES ($1, $2, $3, 'free', 'practice', '{}'::jsonb, 'ended', $4)`,
      [ids.session, ORG_A, deriveLearnerId('streamer-001'), `key-${ids.session}`],
    );
    await db.query(
      `INSERT INTO conversation (id, organization_id, training_session_id, status, version, last_sequence)
       VALUES ($1, $2, $3, 'ended', 1, 1)`,
      [ids.conversation, ORG_A, ids.session],
    );
  } else {
    await db.query(
      `INSERT INTO assignment (id, organization_id, release_snapshot_id, name, status, starts_at, ends_at, max_attempts, target_principal_ids)
    ...
    await db.query(
      ...
      [ids.learnerAssignment, ORG_A, assignmentId, deriveLearnerId('streamer-001'), ids.attempt],
    );
    await db.query(
      `INSERT INTO training_attempt (id, organization_id, learner_assignment_id, idempotency_key, status)
       VALUES ($1, $2, $3, $4, 'created')`,
      [ids.attempt, ORG_A, ids.learnerAssignment, `key-${ids.attempt}`],
    );
    await db.query(
      `INSERT INTO training_session (id, organization_id, learner_id, source_type, mode, release_snapshot_id, assignment_id, learner_assignment_id, training_attempt_id, persona_snapshot, status, idempotency_key)
       VALUES ($1, $2, $3, 'assigned', 'practice', '11111111-1111-1111-1111-111111111120', $4, $5, $6, '{}'::jsonb, 'ended', $7)`,
      [ids.session, ORG_A, deriveLearnerId('streamer-001'), assignmentId, ids.learnerAssignment, ids.attempt, `key-${ids.session}`],
    );
    await db.query(
      `INSERT INTO conversation (id, organization_id, training_attempt_id, release_snapshot_id, training_session_id, status, version, last_sequence)
       VALUES ($1, $2, $3, '11111111-1111-1111-1111-111111111120', $4, 'ended', 1, 1)`,
      [ids.conversation, ORG_A, ids.attempt, ids.session],
    );
  }
  await db.query(
    `INSERT INTO conversation_message (id, organization_id, conversation_id, sequence, client_message_id, role, content, request_hash)
     VALUES ($1, $2, $3, 1, 'm-1', 'learner', 'hello', 'h1')`,
    [messageId, ORG_A, ids.conversation],
  );
  await db.query(
    `INSERT INTO evaluation_job (id, organization_id, conversation_id, status, attempt_count)
     VALUES ($1, $2, $3, 'queued', 0)`,
    [ids.job, ORG_A, ids.conversation],
  );
  await db.query(
    `INSERT INTO outbox_event (id, organization_id, aggregate_type, aggregate_id, event_type, payload, deduplication_key, status)
     VALUES ($1, $2, 'evaluation', $3, 'evaluation.requested', $4::jsonb, $5, 'pending')`,
    [ids.event, ORG_A, ids.conversation, JSON.stringify({ conversationId: ids.conversation, jobId: ids.job }), `evaluation:${ids.conversation}`],
  );
  await db.close();
}
...
test('free evaluation writes back the free counter/score once, aggregates dimensions and scores the session', { timeout: 120_000 }, async (context) => {
  ...
  const profile = await db.query<{ total_free: number; total_assigned: number; avg: string; dims: Record<string, { score: number; samples: number }>; weak: string[] }>(
    `SELECT total_free_sessions AS "total_free", total_assigned_sessions AS "total_assigned",
            avg_score::text AS avg, dimension_scores AS dims, weak_points AS weak
     FROM learner_profile WHERE internal_learner_id = $1 AND organization_id = $2`,
    [deriveLearnerId('streamer-001'), ORG_A],
  );
  ...
  const session = await db.query<{ status: string }>('SELECT status FROM training_session WHERE id = $1', [ids.session]);
  ...
  const after = await db.query<{ total_free: number }>('SELECT total_free_sessions AS "total_free" FROM learner_profile WHERE internal_learner_id = $1', [deriveLearnerId('streamer-001')]);
