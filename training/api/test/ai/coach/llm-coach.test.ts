...
import type { CoachFeedbackParams } from '../../../src/ai/coach/rule-coach.js';

test('coach prompt reviews the learner against the previous customer message without seeing the generated reply', () => {
  const params: CoachFeedbackParams & {
    readonly previousCustomerMsg: string;
    readonly conversationContext: readonly { readonly role: 'learner' | 'assistant'; readonly content: string }[];
  } = {
    userMsg: '我推荐您使用外涂护肤品。',
