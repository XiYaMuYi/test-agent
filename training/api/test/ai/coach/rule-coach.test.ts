...
import type { DetectedEvent } from '../../../src/ai/coach/event-detector.js';

// ── 辅助函数 ──────────────────────────────────────────────

function defaultParams(overrides: Partial<Parameters<typeof ruleBasedCoachFeedback>[0]> = {}) {
  return {
...
test('need_revealed 事件 → 加 8', () => {
  const events: DetectedEvent[] = [{ type: 'need_revealed', detail: '我想', quality: 70 }];
...
test('objection_raised 事件 → 扣 5', () => {
  const events: DetectedEvent[] = [{ type: 'objection_raised', detail: '太贵' }];
  const result = ruleBasedCoachFeedback(defaultParams({ events }));
  assert.ok(result.feedback.includes('异议'));
});

test('trust_built 事件 → 加 10', () => {
  const events: DetectedEvent[] = [{ type: 'trust_built', detail: '专业', quality: 85 }];
  const result = ruleBasedCoachFeedback(defaultParams({ events }));
...
test('closing_signal 事件 → 加 15', () => {
  const events: DetectedEvent[] = [{ type: 'closing_signal', detail: '下单', quality: 90 }];
  const result = ruleBasedCoachFeedback(defaultParams({ events }));
