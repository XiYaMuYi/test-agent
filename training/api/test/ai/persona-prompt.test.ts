...
test('难度1-4指令正确生成', () => {
  const d1 = buildPersonaSystemSection(
    basePersona({ conversation: { difficulty: 1, maxTurns: 20, background: '', productScenario: '护肤' } }),
  );
  assert.match(d1, /1-2个问题后就决定购买/);

  const d2 = buildPersonaSystemSection(
    basePersona({ conversation: { difficulty: 2, maxTurns: 20, background: '', productScenario: '护肤' } }),
  );
  assert.match(d2, /提出一些常见疑虑/);

  const d3 = buildPersonaSystemSection(
    basePersona({ conversation: { difficulty: 3, maxTurns: 20, background: '', productScenario: '护肤' } }),
  );
  assert.match(d3, /频繁对比竞品品牌/);

  const d4 = buildPersonaSystemSection(
    basePersona({ conversation: { difficulty: 4, maxTurns: 20, background: '', productScenario: '护肤' } }),
  );
  assert.match(d4, /几乎不可能被说服/);
});

test('wait_learner时包含"等对方先开口"', () => {
  const out = buildPersonaSystemSection(
    basePersona({
      conversation: {
        difficulty: 2,
        maxTurns: 20,
...
for (const [label, input] of [
  ...
  test(`残缺画像快照不抛错并安全降级：${label}`, () => {
    let out: string;
    assert.doesNotThrow(() => {
      out = buildPersonaSystemSection(input as unknown as PersonaConfig);
    });
...
test('信任度 1-2 级注入先建信任约束，且不出现在 4-5 级', () => {
  const low = buildPersonaSystemSection(
    basePersona({ basic: { maritalStatus: 'married', incomeLevel: 'medium', customerRelation: 'prospect', trustLevel: 1 } }),
  );
  assert.match(low, /戒备心强/);
  assert.match(low, /不急于推销/);
  const high = buildPersonaSystemSection(
    basePersona({ basic: { maritalStatus: 'married', incomeLevel: 'medium', customerRelation: 'returning', trustLevel: 5 } }),
  );
