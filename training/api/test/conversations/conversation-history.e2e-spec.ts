...
async function holdFinishedConversation(app: INestApplication, key: string) {
...
test('learner lists own conversations with persona summary, ordered newest first, and never sees another learner', { timeout: 120_000 }, async (context) => {
  ...
  await db.query(
    `INSERT INTO evaluation_report (id, organization_id, conversation_id, evaluation_job_id, status, report)
     ...
     FROM evaluation_job WHERE conversation_id = $1`,
    [secondId],
  );
  const scored = await request(app.getHttpServer())
    ...
    .set('x-access-token', STREAMER)
    .expect(200);
  const scoredItem = scored.body.items.find((i: { conversationId: string }) => i.conversationId === secondId);
  ...
  assert.ok(!foreign.body.items.some((i: { conversationId: string }) => i.conversationId === firstId));
...
test('learner opens a conversation detail with an ordered learner/assistant transcript', { timeout: 120_000 }, async (context) => {
  ...
  assert.deepEqual(
    detail.body.messages.map((m: { sequence: number }) => m.sequence),
    [1, 1, 2, 2],
    'messages interleave within each turn and stay ordered by sequence',
  );
