...
import { FakeModelAdapter, type FakeModelScenario } from '../../src/adapters/fake-model.adapter.js';
...
async function createApp(databaseUrl: string, modelScenario: FakeModelScenario = 'success') {
...
function withoutAsyncCoachFeedback(value: Record<string, unknown>) {
  const { coachFeedback: _rootFeedback, suggestion, ...stable } = value;
  if (typeof suggestion !== 'object' || suggestion === null) return stable;
  const { coachFeedback: _suggestionFeedback, ...stableSuggestion } = suggestion as Record<string, unknown>;
  return { ...stable, suggestion: stableSuggestion };
}

async function seedConversation(dbUrl: string) {
...
test('conversation flow enforces idempotency, sequence and ownership', { timeout: 120_000 }, async (context) => {
  ...
  const persistedResponse = await db.query<{ response: string | null }>(
    `SELECT response_hash AS response
     FROM conversation_message
     WHERE conversation_id = $1 AND client_message_id = 'msg-1'`,
    [conversationId],
  );
...
test('ending attempts settles quota once, permits the second attempt, and rejects a third after quota exhaustion', { timeout: 120_000 }, async (context) => {
  ...
  await db.query(
    `UPDATE assignment
     SET max_attempts = 2
     WHERE id = '11111111-1111-1111-1111-111111111130' AND organization_id = $1`,
    [ORGANIZATION_A],
  );
  ...
  const firstSettlement = await db.query<{
    status: string;
  ...
  const afterReplay = await db.query<{
    ...
    `SELECT learner.total_attempts AS "totalAttempts",
    ...
    [CONVERSATION_ID, ORGANIZATION_A, `evaluation:${CONVERSATION_ID}`],
  );
  ...
  const completed = await db.query<{
    ...
    `SELECT attempt.status AS "attemptStatus",
     ...
     JOIN learner_assignment learner
       ON learner.id = attempt.learner_assignment_id
      AND learner.organization_id = attempt.organization_id
     WHERE attempt.id = $1 AND attempt.organization_id = $2`,
    [second.body.attemptId, ORGANIZATION_A],
  );
...
test('concurrent messages can advance a conversation only once for a sequence', { timeout: 120_000 }, async (context) => {
  ...
  const messages = await db.query<{ count: string }>('SELECT COUNT(*)::text AS count FROM conversation_message WHERE conversation_id = $1', [CONVERSATION_ID]);
...
for (const failure of [
  ...
  test(`model ${failure.scenario} leaves one recoverable pending turn and same-key retry completes it`, { timeout: 120_000 }, async (context) => {
    ...
    const stateAfterFailure = await db.query<{
      status: string;
      version: number;
      lastSequence: number;
      messageCount: string;
    ...
    const stateAfterRecovery = await db.query<{
      status: string;
      version: number;
      lastSequence: number;
      messageCount: string;
