...
import { buildDefaultConfig, type TransactionalEvaluationExecutorPort } from '@training/contracts';
...
async function createApp(databaseUrl: string, model?: unknown) {
...
interface ScoringWorker {
  pollOnce(): Promise<{ scanned: number; dispatched: number; succeeded?: number; failed?: number }>;
}

async function createScoringWorker(
  database: TransactionalEvaluationExecutorPort,
  generate: (callIndex: number) => Promise<Record<string, unknown>>,
): Promise<ScoringWorker> {
  let callIndex = 0;
  const loadWorkerModule = new Function('path', 'return import(path)') as (path: string) => Promise<{
    createWorkerModule(options: {
      evaluationExecutor: TransactionalEvaluationExecutorPort;
      evaluationGenerator: { generate: () => Promise<Record<string, unknown>> };
    }): ScoringWorker;
  }>;
...
const citationModel = {
  async generate(request: { sessionId: string }) {
    return {
...
async function seedLearnerProfile(db: ReturnType<typeof createPostgresExecutor>) {
  await db.query(
...
async function seedPersonalTemplate(
  db: ReturnType<typeof createPostgresExecutor>,
  templateId: string,
  knowledgeVersions: readonly string[],
) {
  ...
  await db.query(
    `INSERT INTO training_template
     ...
     VALUES ($1, $2, 'personal', $3, $4, $5::jsonb, $6::jsonb, 'active')`,
    [templateId, ORG_A, LEARNER_A, `tpl-${templateId.slice(0, 8)}`, JSON.stringify(buildDefaultConfig()), JSON.stringify(knowledgeVersions)],
  );
}
...
test('dual-track end-to-end: a free then an assigned session for one learner converge into one scored profile', { timeout: 180_000 }, async (context) => {
  ...
  const worker = await createScoringWorker(db, async (callIndex) =>
    callIndex === 1
  ...
  const freeConv: string = freeStart.body.conversationId;
  const freeSession: string = freeStart.body.sessionId;
  ...
  const snapshotRows = await db.query<{ id: string }>('SELECT id FROM release_snapshot WHERE scenario_draft_id = $1', [draftId]);
  ...
  const assignedStart = await request(http)
    ...
    .expect(201);
  const assignedConv: string = assignedStart.body.conversationId;
  ...
  const profile = await db.query<{
    ...
    avg: string;
    dims: Record<string, { score: number; samples: number }>;
    weak: string[];
  }>(
  ...
  const sessions = await db.query<{ sourceType: string; status: string }>(
    `SELECT source_type AS "sourceType", status FROM training_session
     WHERE learner_id = $1 AND organization_id = $2 ORDER BY started_at`,
    [LEARNER_A, ORG_A],
  );
...
test('template knowledge: a free session started from a template inherits approved knowledge for model citations', { timeout: 120_000 }, async (context) => {
  ...
  const started = await request(http)
    ...
    .send({ templateId })
    .expect(201);
  const convId: string = started.body.conversationId;
  ...
  const turn = await request(http)
    ...
    .expect(201);
  assert.deepEqual(
    turn.body.suggestion.knowledgeReferences,
    ['knowledge-welcome@v1'],
    'the template-approved knowledge enters the allowed citation set and is echoed back',
  );

  const linked = await db.query<{ templateId: string | null }>(
    `SELECT ts.template_id AS "templateId"
