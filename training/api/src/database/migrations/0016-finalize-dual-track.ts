 ...
 */
export const finalizeDualTrackMigration: Migration = {
  id: '0016_finalize_dual_track',
  description:
    'Backfill remaining conversation.training_session_id links and enforce NOT NULL to finalize the dual-track contract.',
  ...
  async up(database): Promise<void> {
    ...
    const orphanResult = await database.query<{ count: string }>(`
    ...
    if (remaining > 0) {
      throw new Error(
        `Cannot finalize dual-track: ${remaining} orphan conversation row(s) still lack training_session_id and have no assigned chain to backfill from; triage manually before re-running.`,
      );
    }
