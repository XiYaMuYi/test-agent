...
import type { ModelProviderPort, PersonaConfig } from '@training/contracts';
...
import { SalesEvaluator, type SalesScore } from './sales-evaluator.js';
...
export interface AdminEvaluationListQuery {
  readonly source?: 'free' | 'assigned';
...
export interface AdminEvaluationListResult {
  readonly items: readonly AdminEvaluationListItem[];
...
function readAssistantReply(serialized: string | null): string | null {
  ...
  try {
    ...
    const replyText = (suggestion as { replyText?: unknown }).replyText;
...
function toAdminEvaluationItem(row: QueryResultRow): AdminEvaluationListItem {
  return {
...
export class EvaluationService implements OnModuleDestroy {
  ...
  private readonly salesEvaluator?: SalesEvaluator;

  public constructor(database: Pool, model?: ModelProviderPort) {
  ...
  async getForLearner(principal: CurrentPrincipal, conversationId: string): Promise<EvaluationReportView | undefined> {
    const { rows } = await this.database.query<EvaluationReportView>(
      `SELECT report.id, report.organization_id AS "organizationId", report.conversation_id AS "conversationId",
       ...
       WHERE report.conversation_id = $1 AND report.organization_id = $2 AND ts.learner_id = $3`,
  ...
  async getPendingStatusForLearner(
    principal: CurrentPrincipal,
    conversationId: string,
  ): Promise<'queued' | 'running' | undefined> {
    const { rows } = await this.database.query<{ status: 'queued' | 'running' }>(
      `SELECT job.status
  ...
  async listForOrganization(principal: CurrentPrincipal): Promise<readonly EvaluationReportView[]> {
    const { rows } = await this.database.query<EvaluationReportView>(
      `SELECT id, organization_id AS "organizationId", conversation_id AS "conversationId",
              evaluation_job_id AS "evaluationJobId", report, created_at AS "createdAt"
       FROM evaluation_report WHERE organization_id = $1 ORDER BY created_at DESC`,
   ...
   */
  async listAdminEvaluations(principal: CurrentPrincipal, query: AdminEvaluationListQuery): Promise<AdminEvaluationListResult> {
    const conditions: string[] = ['er.organization_id = $1'];
    ...
    const countResult = await this.database.query<{ count: string }>(
      `SELECT COUNT(*)::text AS count ${ADMIN_EVALUATION_JOIN} ${whereClause}`,
      params,
    );
    ...
    const listResult = await this.database.query(
      `SELECT ${ADMIN_EVALUATION_COLUMNS} ${ADMIN_EVALUATION_JOIN} ${whereClause}
  ...
  async getConversationReplay(principal: CurrentPrincipal, conversationId: string): Promise<ConversationReplay> {
    const metaResult = await this.database.query(
      `SELECT c.id AS "conversationId", c.status,
    ...
    if (meta === undefined) throw new Error('CONVERSATION_NOT_FOUND');
    ...
    const reportResult = await this.database.query<{ report: Record<string, unknown> }>(
      `SELECT report FROM evaluation_report WHERE conversation_id = $1 AND organization_id = $2`,
      [conversationId, principal.organizationId],
    );
