...
type LeaveStrength = 'strong' | 'mild';
 ...
 */
const LEAVE_PATTERNS: Record<LeaveStrength, string[]> = {
  strong: [
    '再见',
    '拜拜',
    '不买了',
...
export interface CheckCustomerLeaveResult {
  leaving: boolean;
  strength?: LeaveStrength;
  keyword?: string;
  reason?: string;
}
 ...
 */
export function checkCustomerLeave(text: string): CheckCustomerLeaveResult {
  for (const strength of ['strong', 'mild'] as LeaveStrength[]) {
