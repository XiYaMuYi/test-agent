...
export interface CoachPromptParams extends CoachFeedbackParams {
  /** The customer utterance that the learner was responding to. Empty for a learner-first opening. */
  readonly previousCustomerMsg?: string;
  /** Transcript ending at the learner utterance under review; never contains the generated reply that follows it. */
...
export function buildCoachPrompt(params: CoachPromptParams): string {
...
export function parseCoachFeedback(raw: string, params: CoachFeedbackParams): CoachFeedbackResult | null {
  ...
  if (normalizedImprovements.length === 0) return null;
 ...
 */
export async function llmCoachFeedback(
